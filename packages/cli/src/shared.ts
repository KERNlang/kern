...
const GENERATED_HEADER_PATTERN = /^(\/\/\s*@generated by kern) v\d+\.\d+\.\d+(.*)$/m;
...
export function checkVersionDrift(outFilePath: string, sourceFile: string): void {
  ...
  const current = KERN_VERSION.split('.').map(Number) as [number, number, number];
  if (current.some((n) => Number.isNaN(n))) return;
