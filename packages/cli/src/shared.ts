...
function collectSourceTracePoints(ast: IRNode, sourceName: string): SourceTracePoint[] {
...
export function transpileAndWrite(
  ...
  if (result.artifacts && result.artifacts.length > 0 && cfg.structure !== 'flat') {
    ...
    writeGeneratedFileSync(
      outFilePath,
      withGeneratedHeader(withSourceTraceComments(result.code, ast, sourceName), header),
      relSource,
    );
    ...
    if (target === 'ink' && result.artifacts && result.artifacts.length > 0) {
      ...
      if (entryArtifact) {
        ...
        writeGeneratedFileSync(
          entryFilePath,
          withGeneratedHeader(withSourceTraceComments(entryContent, ast, sourceName, { entry: true }), header),
          relSource,
        );
      }
    }
