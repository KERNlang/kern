...
import type { FixSafety, LLMReviewInput, ReviewConfig, ReviewFinding, ReviewReport } from '@kernlang/review';
import {
  analyzeTaint,
  buildLLMPrompt,
...
async function runReviewLocal(args: string[]): Promise<void> {
  ...
  if (!reviewInput) {
    ...
    console.error(
      '       [--write-baseline=file.json] [--json] [--sarif] [--recursive] [--enforce] [--strict-parse] [--fix] [--autofix[=safe|suggested|risky]] [--verify] [--rules-dir <dir>] [--include-generated]',
    );
    console.error('');
