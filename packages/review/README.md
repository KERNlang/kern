...
Kern Review -- 210 rules, taint tracking, OWASP LLM01 coverage, and framework-aware review checks
...
Kern Review ships 210 AST-based rules across base correctness, security, framework, performance, test-quality, null-safety, dead-logic, concept, and taint-aware analysis layers.
...
- React and browser apps: legacy unsafe class lifecycles, event listener cleanup identity mismatches, unsafe browser storage JSON parsing, client-side open redirects from query params, and wildcard `postMessage` target origins.
