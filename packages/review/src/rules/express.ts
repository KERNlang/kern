...
function unvalidatedInput(ctx: RuleContext): ReviewFinding[] {
  ...
  while ((match = reqBodyRegex.exec(fullText)) !== null) {
    ...
    // Skip if req.body is the argument to a validation function on this line
    // e.g., if (!isValidBody(req.body)) or validate(req.body)
    if (/\b(?:isValid\w*|validate\w*|check\w*)\s*\(/.test(lineText)) continue;
    ...
    // Skip if used only in safe operations (cookie setting, static responses)
    if (/\bres\.cookie\s*\(/.test(lineText) || /\bres\.set\s*\(/.test(lineText)) continue;
