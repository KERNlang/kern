...
const LLM_API_PATTERNS =
  /\bgenerateContent\b|\bchat\.completions\.create\b|\bcomplete\b|\bsendMessage\b|\bcreateCompletion\b|\bcreateChatCompletion\b/;
...
const SANITIZER_CALL =
  /\bsanitize\w*\s?\(|\bescape\w*\s?\(|\bclean\w*\s?\(|\bstripDelimiters\s?\(|\bcleanForPrompt\s?\(/;
const VALIDATION_CALL = /\.parse\s*\(|\.safeParse\s*\(|\.validate\s*\(|\.validateSync\s*\(/;
const RETRIEVAL_PATTERNS =
  /\bvectorStore\.search\b|\bvectorDb\.search\b|\bretrieve\b|\bsimilaritySearch\b|\bembedding\.query\b|\bindex\.query\b|\bsemantic[Ss]earch\b|\b\w+[Dd]b\.search\b|\b\w+[Ss]tore\.search\b/;
const EXEC_SINKS =
  /\beval\s*\(|\bnew\s+Function\s*\(|\bvm\.runInContext\s*\(|\bvm\.runInNewContext\s*\(|\bexec\s*\(|\bexecSync\s*\(/;
...
function indirectPromptInjection(ctx: RuleContext): ReviewFinding[] {
  ...
  for (const call of ctx.sourceFile.getDescendantsOfKind(SyntaxKind.CallExpression)) {
    ...
    const sanitizeWraps = new RegExp(
      `\\$\\{[^}]*(?:sanitize\\w*|escape\\w*|cleanForPrompt)\\s*\\([^)]*\\b${ev}\\b`,
    ).test(bodyText);
    ...
    findings.push(
      finding(
        ...
        {
          suggestion:
            'Sanitize DB-sourced content before embedding in LLM prompts — stored data may contain injection payloads',
        },
      ),
    );
  }
      ...
      if (new RegExp(`\\b${v}\\b`).test(argsText)) {
        findings.push(
          finding(
            ...
            {
              suggestion:
                'Never execute LLM output directly. Validate against an allowlist or use a sandboxed interpreter',
            },
          ),
        );
        break;
...
const SYSTEM_PROMPT_VARS =
  /\b(systemPrompt|SYSTEM_PROMPT|system_prompt|sysPrompt|systemInstruction|SYSTEM_INSTRUCTION)\b/;
  ...
  for (const call of ctx.sourceFile.getDescendantsOfKind(SyntaxKind.CallExpression)) {
    ...
    findings.push(
      finding(
        ...
        {
          suggestion:
            'Sanitize retrieved content before embedding in prompts — indexed documents may contain injection payloads',
        },
      ),
    );
  }
...
function toolCallingManipulation(ctx: RuleContext): ReviewFinding[] {
  ...
  if (USER_INPUT_PATTERNS.test(fileText) && hasToolProps)
    for (const objLit of ctx.sourceFile.getDescendantsOfKind(SyntaxKind.ObjectLiteralExpression)) {
      ...
      for (const prop of objLit.getProperties()) {
        ...
        const isUserInput =
          USER_INPUT_PATTERNS.test(valueText) || /\b(userInput|input|query|message)\b/.test(valueText);
  ...
  for (const forOf of ctx.sourceFile.getDescendantsOfKind(SyntaxKind.ForOfStatement)) {
    ...
    if (/\b(execute|call|invoke|run|dispatch)\w*\s?\(/i.test(forBody) && /\.name\b|\.function\b/.test(forBody)) {
      // Check if there's an allowlist/validation on the tool NAME before execution
      if (
        !/allowlist\w*\.has\s?\(\s?\w+\.name|whitelist\w*\.has\s?\(\s?\w+\.name|allowed\w*\.includes\s?\(\s?\w+\.name|validTools\w*\.has\s?\(\s?\w+\.name/.test(
          forBody,
        )
      ) {
        findings.push(
  ...
  for (const call of ctx.sourceFile.getDescendantsOfKind(SyntaxKind.CallExpression)) {
    ...
    const argsText = call
      ...
      .map((a) => a.getText())
      .join(' ');
    if (/\b(execute|call|invoke|run|dispatch)\w*\s?\(/i.test(argsText) && /\.name\b|\.function\b/.test(argsText)) {
      if (
        !/allowlist\w*\.has\s?\(\s?\w+\.name|whitelist\w*\.has\s?\(\s?\w+\.name|allowed\w*\.includes\s?\(\s?\w+\.name|validTools\w*\.has\s?\(\s?\w+\.name/.test(
          argsText,
        )
...
const DECODE_PATTERNS =
  /\batob\s*\(|\bBuffer\.from\s*\([^)]*(['"]base64['"]|['"]hex['"])|\bdecodeURIComponent\s*\(|\bdecodeURI\s*\(/;
...
function encodingBypass(ctx: RuleContext): ReviewFinding[] {
  ...
  for (const call of ctx.sourceFile.getDescendantsOfKind(SyntaxKind.CallExpression)) {
    ...
    const decodeFn = callText.includes('atob')
      ? 'atob()'
      : callText.includes('Buffer.from')
        ? 'Buffer.from()'
        : 'decodeURIComponent()';
    ...
    findings.push(
      finding(
        ...
        {
          suggestion:
            'Sanitize decoded content before embedding in prompts — base64/hex encoding can bypass input filters',
        },
      ),
    );
  }
...
const DELIMITER_PATTERN =
  /`{3}|---|###|<\/?system>|<\/?instruction>|<\/?user>|<\/?assistant>|<\|system\|>|<\|user\|>|<\|end\|>|<\|assistant\|>|\[INST\]|\[\/INST\]/;
      ...
      if (isUserControlled) {
        findings.push(
          finding(
            ...
            {
              suggestion:
                'Strip or escape delimiters (```, ---, ###, XML tags) from user input before embedding in structured prompts',
            },
          ),
        );
        break;
...
function missingOutputValidation(ctx: RuleContext): ReviewFinding[] {
  ...
  for (const [varName, declLine] of llmVars) {
    ...
    const fnBody = ctx.sourceFile
      .getDescendantsOfKind(SyntaxKind.Block)
      .find((b) => b.getStartLineNumber() <= declLine && b.getEndLineNumber() >= declLine);
    if (!fnBody) continue;
    const bodyText = fnBody.getText();
    ...
    const validationMatch = new RegExp(
      `\\w+Schema\\.parse\\s*\\(\\s*${ev}|validate\\w*\\s*\\(\\s*${ev}|\\.parse\\s*\\(\\s*${ev}\\s*\\)|\\.safeParse\\s*\\(\\s*${ev}\\s*\\)`,
    ).exec(bodyText);
