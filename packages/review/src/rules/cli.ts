...
function isEntrypointLike(ctx: RuleContext, text: string): boolean {
  return (
    /(?:^|\/)(?:cli|bin|main|index)\.[cm]?[jt]sx?$/.test(ctx.filePath) ||
    text.startsWith('#!') ||
...
function hasCommandInstance(ctx: RuleContext): boolean {
  return ctx.sourceFile
    ...
    .some((expr) => expr.getExpression().getText() === 'Command');
}
...
function processExitInAction(ctx: RuleContext): ReviewFinding[] {
  ...
  for (const call of ctx.sourceFile.getDescendantsOfKind(SyntaxKind.CallExpression)) {
    ...
    const exitCall = callback.getDescendantsOfKind(SyntaxKind.CallExpression).find((desc) => {
      ...
      return (
        Node.isPropertyAccessExpression(descExpr) &&
...
export const cliRules: ReviewRule[] = [missingShebang, missingParse, asyncParseSync, processExitInAction];
