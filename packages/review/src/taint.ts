...
const SANITIZER_PATTERNS = [
  ...
  // URL allowlist validation (new URL + hostname/origin check)
  { pattern: /\bnew\s+URL\s*\(/, name: 'URL allowlist' },
];
...
const SANITIZER_SUFFICIENCY: Record<string, Set<SinkCategory>> = {
  ...
  'URL allowlist':            new Set(['redirect']),
  'URL':                      new Set(['redirect']),
};
...
function findSanitizersAST(body: Node, taintedNames: Set<string>): Array<{ name: string; sanitizedVars: Set<string> }> {
  ...
  // Also scan NewExpression nodes (e.g., new URL(returnTo) is a sanitizer for redirect)
  body.forEachDescendant(n => {
    if (n.getKindName() !== 'NewExpression') return;
    const newExpr = n as import('ts-morph').NewExpression;
    const ctorName = newExpr.getExpression().getText();
    const matchedSanitizer = SANITIZER_PATTERN_NAMES.find(s => ctorName.includes(s));
    if (!matchedSanitizer) return;
    ...
    // If result is assigned to a variable, that variable replaces the tainted value
