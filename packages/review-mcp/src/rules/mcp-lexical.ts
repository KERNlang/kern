...
export function createLexicalMask(source: string): string {
  ...
  for (let i = 0; i < chars.length; i++) {
    ...
    if (inTemplate) {
      if (ch === '\\' && i + 1 < chars.length) { chars[i] = ' '; chars[i + 1] = ' '; i++; continue; }
    ...
    if (templateStack.length > 0) {
      if (ch === '{') { templateStack[templateStack.length - 1]++; continue; }
      if (ch === '}') {
    ...
    if (ch === '/' && next === '/') { inLineComment = true; chars[i] = ' '; continue; }
    if (ch === '/' && next === '*') { inBlockComment = true; chars[i] = ' '; chars[i + 1] = ' '; i++; continue; }
    if (ch === '#' && (i === 0 || source[i - 1] === '\n' || /^\s*$/.test(source.slice(source.lastIndexOf('\n', i - 1) + 1, i)))) { inLineComment = true; chars[i] = ' '; continue; }
