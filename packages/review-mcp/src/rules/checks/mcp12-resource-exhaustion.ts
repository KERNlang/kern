...
export function resourceExhaustion(source: string, filePath: string): ReviewFinding[] {
  ...
  for (const region of regions) {
    ...
    if (isPython) {
      if (
        /\b(requests\.(get|post|put|delete|patch|head)\s*\(|httpx\.\w+\s*\()/.test(code) &&
        !/timeout\s*=/.test(code)
      ) {
        findings.push(
          finding(
      ...
      if (
        /\b(cursor\.(execute|fetchall)|\.execute\s*\()/.test(code) &&
        !/timeout|statement_timeout|connect_timeout/.test(code)
      ) {
        findings.push(
          finding(
            'mcp-resource-exhaustion',
