...
describe('golden: fn', () => {
  ...
  it('stream function', () => {
    expect(gen([
      'fn name=generate returns=string stream=true',
      '  handler <<<',
      '    yield "hello";',
      '  >>>',
    ].join('\n'))).toMatchSnapshot();
...
describe('golden: const', () => {
  ...
  it('const with handler', () => {
    expect(gen([
      'const name=CONFIG type=AppConfig',
      '  handler <<<',
...
describe('golden: assume', () => {
  it('assume with evidence and fallback', () => {
    expect(gen('assume expr={{user.id === params.userId}} scope=request evidence="route-signing" fallback="throw AuthError()"')).toMatchSnapshot();
  });
});
...
describe('golden: each', () => {
  it('each with child nodes', () => {
    expect(gen([
      ...
      '  derive name=processed expr={{transform(item)}} type=Result',
    ].join('\n'))).toMatchSnapshot();
  });
  it('each with index', () => {
...
describe('golden: collect', () => {
  it('collect with filter, sort, limit', () => {
    expect(gen('collect name=topItems from="items" where={{item.score > 50}} order={{b.score - a.score}} limit=10')).toMatchSnapshot();
  });
  it('collect simple', () => {
