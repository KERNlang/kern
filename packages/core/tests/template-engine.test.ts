...
function expand(source: string): string {
  const ast = parse(source);
  const _node = ast.type === 'template' ? ast : ast;
...
function _expandChild(source: string): string {
  const ast = parse(source);
...
describe('Template Engine', () => {
  ...
  describe('registration', () => {
    ...
    it('clearTemplates resets registry', () => {
      reg(['template name=test-tmpl', '  slot name=x type=expr', '  body <<<', '    {{x}}', '  >>>'].join('\n'));
      expect(templateCount()).toBe(1);
      clearTemplates();
  ...
  describe('expansion', () => {
    ...
    it('expands {{CHILDREN}} with handler blocks', () => {
      ...
      const code = expand(
        ['wrapper name=doStuff', '  handler <<<', '    console.log("hello");', '    return 42;', '  >>>'].join('\n'),
      );
    ...
    it('expands core node children via codegen pipeline', () => {
      ...
      const code = expand(['module-tmpl name=myModule', '  type name=Status values="ok|error"'].join('\n'));
  ...
  describe('codegen integration', () => {
    it('generateCoreNode returns [] for template definitions', () => {
      const ast = parse(
        ['template name=my-template', '  slot name=x type=expr', '  body <<<', '    {{x}}', '  >>>'].join('\n'),
      );
    ...
    it('template + core nodes in same file work together', () => {
      ...
      const source = ['type name=Status values="ok|error"', '  helper-fn name=getDefault value="\'ok\'"'].join('\n');
      const ast = parse(source);
  ...
  describe('real-world: arrow-fn template', () => {
    ...
    it('expands with minimal slots', () => {
      const code = expand(['arrow-fn name=noop', '  handler <<<', '    // nothing', '  >>>'].join('\n'));
  ...
  describe('recursion guard', () => {
    it('throws on excessive nesting depth', () => {
      ...
      reg(['template name=deep-tmpl', '  slot name=x type=expr', '  body <<<', '    {{x}}', '  >>>'].join('\n'));
