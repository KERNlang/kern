...
describe('expression-v1 structural constitution admission', () => {
  test('round-trips an exact binary expression and inflates it without precedence drift', () => {
    ...
    expect(artifact.format).toBe('kern.kir.structural.r1.5f.1-alpha');
    expect(artifact.constitution).toBe('kern.kir.structural.r1.5f.1');
  ...
  test('keeps optional type and the next two required raw expressions excluded', () => {
    ...
    expectStructuralCode(
      () => encodeStructuralKir({ type: 'branch', props: { name: 'choice', on: 'answer' } }, limits),
      'excluded-host-payload',
    );
    expectStructuralCode(
      () => encodeStructuralKir({ type: 'each', props: { in: '[1]', name: 'item' } }, limits),
      'excluded-host-payload',
    );
  });
  ...
  test('reader rejects the predecessor artifact format without fallback', () => {
    ...
    format.value = 'kern.kir.structural.r1.5e.1-alpha';
