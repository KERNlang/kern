...
describe('Schema Validation', () => {
  ...
  describe('allowed children', () => {
    ...
    it('allows state and transition in machine', () => {
      const v = validate([
        'machine name=Order',
        '  state name=pending',
        '  transition name=start from=pending to=running',
      ].join('\n'));
      expect(v).toHaveLength(0);
    });
  });
  ...
  describe('complex valid nodes', () => {
    ...
    it('passes valid action', () => {
      const v = validate([
        'action name=notify params="to:string" returns=void idempotent=true',
        '  handler <<<await send(to);>>>',
      ].join('\n'));
      expect(v).toHaveLength(0);
    });
  });
