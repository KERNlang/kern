export const TARGET_EXECUTION_SOURCE = `
  ...
  const __operandFault = () => {
    throw new __Fault('unsupported-runtime-input', 'execution', 'KIR_BINARY_OPERAND_TYPE');
  };
  const __boolOperand = (operand) => {
    if (operand.tag !== 'boolean') __operandFault();
    return operand;
  };
  const __intOperand = (operand) => {
    if (operand.tag !== 'integer') __operandFault();
