...
export const NODE_SCHEMAS: Record<string, NodeSchema> = {
  ...
  step: {
    ...
    description:
      'Finally clause of a `try` block — runs after the try (and catch, if present) regardless of whether an exception was raised. Body-statement form only: must be a direct child of a body-statement `try` (parent `handler lang="kern"`). The `try` parent must contain at least one of `catch` or `finally`. Use for cleanup that must always run (close handles, restore state). Propagation `?` is rejected here for the same reason it is rejected inside `try` — the hoisted err-branch `return` would suppress the original exception/return. Bind to a `let` outside the `try` if you need conditional fallthrough.',
  ...
  clamp: {
    description:
      'Declarative numeric clamp binding — `clamp name=bounded value=score min=0 max=100` lowers to a cross-target clamp expression bounded by `min`/`max` expressions. In native handler bodies it emits a local binding and avoids raw host-language clamp idioms.',
    example: 'clamp name=bounded value=score min=0 max=100',
    props: {
      name: { required: true, kind: 'identifier' },
      value: { required: true, kind: 'expression' },
      min: { required: true, kind: 'expression' },
      max: { required: true, kind: 'expression' },
      type: { kind: 'typeAnnotation' },
      trailingComment: { kind: 'string' },
      export: { kind: 'boolean' },
    },
