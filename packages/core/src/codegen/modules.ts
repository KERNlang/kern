...
function isPtySessionSidecar(manifest: SidecarManifest): boolean {
  const protocol = manifest.protocol?.toLowerCase();
  return protocol === 'pty-session' || protocol === 'ptysession' || protocol === 'session';
}

function generatePythonPtySessionClient(manifest: SidecarManifest, node: IRNode): string[] {
  const baseName = emitIdentifier(lowerFirst(manifest.name), 'pythonPtySession', node);
  const moduleName = manifest.module ?? manifest.packages[0]?.package ?? manifest.name;
  const defaultArgs = manifest.args ?? [];
  const sessionName = emitIdentifier(manifest.session ?? `${upperFirst(baseName)}Session`, 'ptySession', node);
  const errorName = emitIdentifier(manifest.error ?? `${upperFirst(baseName)}SessionError`, 'ptySessionError', node);
  const timeoutName = emitIdentifier(
    manifest.timeout ?? `${upperFirst(baseName)}SessionTimeout`,
    'ptySessionTimeout',
    node,
  );
  const modeName = emitIdentifier(`${upperFirst(sessionName)}Mode`, 'ptySessionMode', node);
  const optionsName = emitIdentifier(
    manifest.options ?? `${upperFirst(sessionName)}SpawnOptions`,
    'ptySessionSpawnOptions',
    node,
  );
  const pendingName = emitIdentifier(`${upperFirst(sessionName)}PendingReply`, 'ptySessionPendingReply', node);
  const daemonMessageName = emitIdentifier(`${upperFirst(sessionName)}DaemonMessage`, 'ptySessionDaemonMessage', node);
  const withTimeoutName = emitIdentifier(`with${upperFirst(sessionName)}Timeout`, 'ptySessionTimeoutHelper', node);
  ...
  return [
    ...
    '    super(message);',
    `    this.name = ${JSON.stringify(errorName)};`,
    '    this.kind = kind;',
    '  }',
    '}',
    '',
    `export class ${timeoutName} extends ${errorName} {`,
    "  constructor(message = 'session timed out') {",
    "    super('timeout', message);",
    `    this.name = ${JSON.stringify(timeoutName)};`,
    '  }',
    '}',
    '',
    `export type ${modeName} = 'exec' | 'agent';`,
    '',
    ...
    `  reject: (err: ${errorName}) => void;`,
    '  onChunk?: (delta: string) => void;',
    '}',
    '',
    `type ${daemonMessageName} =`,
    "  | { type: 'ready'; engine?: string }",
    "  | { type: 'chunk'; id: number; delta: string }",
    "  | { type: 'reply'; id: number; ok: true; text: string }",
    "  | { type: 'reply'; id: number; ok: false; kind?: 'timeout' | 'error'; error: string };",
    '',
    `export class ${sessionName} {`,
    '  private nextId = 1;',
    `  private readonly pending = new Map<number, ${pendingName}>();`,
    ...
    "    private readonly rl: import('node:readline').Interface,",
    '  ) {',
    ...
    `      const wrapped = err instanceof ${errorName} ? err : new ${errorName}('engine', err.message || String(err));`,
    '      this.closed = true;',
    ...
    '      this.closed = true;',
    `      this.failAllPending(new ${errorName}('engine', detail));`,
    '      if (!wasClosed) readyReject(new Error(detail));',
    '    });',
    "    child.stdin.on('error', fail);",
    "    child.stdout.on('error', fail);",
    ...
    '  }',
    '',
    `  static async spawn(opts: ${optionsName} = {}): Promise<${sessionName}> {`,
    "    const py = opts.pythonBin ?? process.env.KERN_PYTHON ?? process.env.PYTHON ?? 'python3';",
    `    const args = ['-u', '-m', ${JSON.stringify(moduleName)}, ...(${JSON.stringify(defaultArgs)}), ...(opts.args ?? [])];`,
    ...
    '    const env = opts.env ? ({ ...process.env, ...opts.env } as Record<string, string>) : (process.env as Record<string, string>);',
    '    let child: import("node:child_process").ChildProcessWithoutNullStreams;',
    '    try {',
    "      child = spawn(py, args, { stdio: ['pipe', 'pipe', 'pipe'], cwd: opts.cwd ?? process.cwd(), env });",
    '    } catch (e) {',
    `      throw new ${errorName}('spawn', \`failed to spawn \${py}: \${e instanceof Error ? e.message : String(e)}\`);`,
    '    }',
    '',
    '    const rl = createInterface({ input: child.stdout });',
    `    const session = new ${sessionName}(${JSON.stringify(defaultArgs[0] ?? manifest.name)}, child, rl);`,
    '    const timeoutMs = opts.daemonTimeoutMs ?? 35_000;',
    '    try {',
    `      await ${withTimeoutName}(session.readyPromise, timeoutMs, 'daemon ready');`,
    '    } catch (e) {',
    '      session.killImmediate();',
    `      if (e instanceof ${errorName}) throw e;`,
    `      throw new ${errorName}('timeout', e instanceof Error ? e.message : String(e));`,
    '    }',
    '    return session;',
    '  }',
    '',
    '  async ask(prompt: string, timeoutMs = 60_000): Promise<string> {',
    `    if (this.closed) throw new ${errorName}('engine', 'session is closed');`,
    `    if (this.child.exitCode !== null || this.child.signalCode !== null) throw new ${errorName}('engine', \`session process exited code=\${this.child.exitCode} signal=\${this.child.signalCode}\`);`,
    `    if (this.busy) throw new ${errorName}('engine', 'session is busy (concurrent ask() forbidden)');`,
    '    this.busy = true;',
    '    const id = this.nextId++;',
    '    try {',
    '      return await new Promise<string>((resolve, reject) => {',
    '        let ceiling: ReturnType<typeof setTimeout> | undefined;',
    '        const cleanup = () => { if (ceiling) clearTimeout(ceiling); };',
    '        this.pending.set(id, {',
    ...
    '          cleanup();',
    `          reject(err instanceof Error ? new ${errorName}('engine', err.message) : new ${errorName}('engine', String(err)));`,
    '        });',
    '        ceiling = setTimeout(() => {',
    '          const pending = this.pending.get(id);',
    '          if (pending) {',
    '            this.pending.delete(id);',
    `            pending.reject(new ${timeoutName}(\`daemon did not reply within \${timeoutMs + 2000}ms\`));`,
    '          }',
    '        }, timeoutMs + 2000);',
    '        ceiling.unref();',
    '      });',
    ...
    '  }',
    '',
    '  async *askStream(prompt: string, timeoutMs = 60_000): AsyncGenerator<string, string, void> {',
    `    if (this.closed) throw new ${errorName}('engine', 'session is closed');`,
    `    if (this.child.exitCode !== null || this.child.signalCode !== null) throw new ${errorName}('engine', \`session process exited code=\${this.child.exitCode} signal=\${this.child.signalCode}\`);`,
    `    if (this.busy) throw new ${errorName}('engine', 'session is busy (concurrent ask() forbidden)');`,
    '    this.busy = true;',
    ...
    '    const wake = () => { const s = signal; if (s) { signal = null; s(); } };',
    '    let replyResolve!: (text: string) => void;',
    `    let replyReject!: (err: ${errorName}) => void;`,
    '    type ReplyState = { ok: true; text: string } | { ok: false; err: Error };',
    '    let replyState: ReplyState | null = null;',
    '    const replyPromise = new Promise<string>((res, rej) => { replyResolve = res; replyReject = rej; });',
    '    replyPromise.then(',
    '      (text) => { replyState = { ok: true, text }; wake(); },',
    `      (err) => { replyState = { ok: false, err: err instanceof Error ? err : new ${errorName}('engine', String(err)) }; wake(); },`,
    '    );',
    '    this.pending.set(id, { resolve: replyResolve, reject: replyReject, onChunk: (delta) => { queue.push(delta); wake(); } });',
    "    this.child.stdin.write(`${JSON.stringify({ id, type: 'ask_stream', prompt, timeout: timeoutMs / 1000 })}\\n`, (err) => {",
    '      if (!err) return;',
    '      this.pending.delete(id);',
    `      replyReject(err instanceof Error ? new ${errorName}('engine', err.message) : new ${errorName}('engine', String(err)));`,
    '    });',
    '    const ceiling = setTimeout(() => {',
    '      if (this.pending.has(id)) {',
    '        this.pending.delete(id);',
    `        replyReject(new ${timeoutName}(\`daemon did not reply within \${timeoutMs + 2000}ms\`));`,
    '      }',
    '    }, timeoutMs + 2000);',
    '    ceiling.unref();',
    '    try {',
    ...
    '    this.closed = true;',
    '    try {',
    "      if (this.child.stdin.writable) this.child.stdin.write(`${JSON.stringify({ id: 0, type: 'close' })}\\n`);",
    ...
    '    }',
    '    if (this.child.exitCode === null) this.killImmediate();',
    '  }',
    '',
    '  private killImmediate(): void {',
    "    try { this.child.kill('SIGTERM'); } catch {}",
    "    setTimeout(() => { if (this.child.exitCode === null) { try { this.child.kill('SIGKILL'); } catch {} } }, 500).unref();",
    '  }',
    '',
    `  private failAllPending(err: ${errorName}): void {`,
    ...
    `      else pending.reject(msg.kind === 'timeout' ? new ${timeoutName}(msg.error) : new ${errorName}('engine', msg.error));`,
    '    }',
    '  }',
    '',
    ...
    '}',
    '',
    `async function ${withTimeoutName}<T>(promise: Promise<T>, ms: number, label: string): Promise<T> {`,
    '  let timer: ReturnType<typeof setTimeout> | undefined;',
    '  try {',
    '    return await Promise.race([',
    '      promise,',
    `      new Promise<T>((_, reject) => { timer = setTimeout(() => reject(new ${timeoutName}(\`\${label}: exceeded \${ms}ms\`)), ms); }),`,
    '    ]);',
    '  } finally {',
...
export function generateIsland(node: IRNode): string[] {
  ...
  let sidecarClient: string[] = [];
  if (manifest) {
    if (manifest.protocol && !isPtySessionSidecar(manifest)) {
      throw new KernCodegenError(`Unsupported sidecar protocol '${manifest.protocol}'. Expected 'pty-session'.`, node);
    }
    sidecarClient = isPtySessionSidecar(manifest)
      ? generatePythonPtySessionClient(manifest, node)
      : generatePythonSidecarClient(manifest, node);
  }
