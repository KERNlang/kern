...
export function collectShapeRegistry(rootOrNodes: IRNode | readonly IRNode[]): ShapeRegistry {
...
export function resolveShape(shape: ShapeInterface, registry: ShapeRegistry, stack: readonly string[]): ResolvedShape {
  const diagnostics: CoreShapeDiagnostic[] = [];
  ...
  for (const baseName of shape.extendsNames) {
    ...
    diagnostics.push(...interfaceDuplicateDiagnostics(registry, baseName));
...
export function interfaceDuplicateDiagnostics(
  registry: ShapeRegistry,
  interfaceName: string,
): readonly CoreShapeDiagnostic[] {
...
export function isSimpleIdentifier(type: string): boolean {
...
export function dedupeDiagnostics(diagnostics: readonly CoreShapeDiagnostic[]): CoreShapeDiagnostic[] {
...
function interfaceNodes(rootOrNodes: IRNode | readonly IRNode[]): readonly IRNode[] {
  const found: IRNode[] = [];
  for (const node of Array.isArray(rootOrNodes) ? rootOrNodes : [rootOrNodes]) visitInterfaceNodes(node, found);
  return found;
}
