...
const PYTHON_STDLIB_SIGNATURES: Record<string, ExternalSignatureMap> = {
  builtins: {
...
export function inferExternalSignature(
  ...
  if (registry !== 'pypi') return undefined;
...
export function inferExternalSignatureMap(
  registry: ExternalImportRegistry,
  packageName: string,
): ExternalSignatureMap | undefined {
  if (registry !== 'pypi') return undefined;
  const signatures = PYTHON_STDLIB_SIGNATURES[packageName];
  return signatures ? { ...signatures } : undefined;
}

export function mergeExternalSignatureMaps(
  inferred: ExternalSignatureMap | undefined,
  explicit: ExternalSignatureMap | undefined,
): ExternalSignatureMap | undefined {
  const merged = { ...(inferred ?? {}), ...(explicit ?? {}) };
  return Object.keys(merged).length > 0 ? merged : undefined;
}

function sanitizeSignatureMap(value: Record<string, unknown>): ExternalSignatureMap | undefined {
...
function parseCompactSignatureMap(value: string): ExternalSignatureMap | undefined {
  const out: ExternalSignatureMap = {};
  for (const part of splitCompactSignatureParts(value)) {
...
function splitCompactSignatureParts(value: string): string[] {
  const parts: string[] = [];
  let start = 0;
  let parenDepth = 0;
  let braceDepth = 0;
...
function isSafeSignatureName(name: string): boolean {
