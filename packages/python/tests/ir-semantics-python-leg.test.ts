...
} from '../../core/src/ir/semantics/assign.js';
import {
...
import { _resetTryContractForTest, registerTryContract, tryContract } from '../../core/src/ir/semantics/try.js';
import { _resetWhileContractForTest, registerWhileContract, whileContract } from '../../core/src/ir/semantics/while.js';
...
describeIfPython('Python emitter leg — try fixtures (three-way differential)', () => {
...
describeIfPython('Python emitter leg — while fixtures (three-way differential)', () => {
  it.each(whileContract.fixtures.map((f) => [f.description, f] as const))(
    'fixture: %s',
    async (_desc, fixture) => {
      const result = await runDifferential(fixture, { pythonLeg: runPythonEmitterLeg });
      if (result.verdict !== 'pass') {
...
describeIfPython('Python emitter leg — fmt fixtures (three-way differential)', () => {
  it.each(fmtContract.fixtures.map((f) => [f.description, f] as const))(
    'fixture: %s',
    async (_desc, fixture) => {
      const result = await runDifferential(fixture, { pythonLeg: runPythonEmitterLeg });
      if (result.verdict !== 'pass') {
