...
test('source and built closures reject every frozen dynamic-loader escape family', () => {
  ...
  const mutants = [
    ...
    'const get = (target, key) => target[key]; try { throw 0; } catch (Build) { var Build = get(Math.max, "constructor"); Build("return process")(); }',
    'const get = (target, key) => target[key]; for (const Build of [get(Math.max, "constructor")]) Build("return process")()',
    'const get = (target, key) => target[key]; let Build; for (Build of [get(Math.max, "constructor")]) Build("return process")()',
    'const get = (target, key) => target[key]; for (const [Build] of [[get(Math.max, "constructor")]]) Build("return process")()',
    'const get = (target, key) => target[key]; async function leak() { for await (const Build of [get(Math.max, "constructor")]) Build("return process")(); } leak()',
    'const get = (target, key) => target[key]; for (const Build of [...[get(Math.max, "constructor")]]) Build("return process")()',
    'const get = (target, key) => target[key]; const [Build] = [...[get(Math.max, "constructor")]]; Build("return process")()',
    'const get = (target, key) => target[key]; const pick = (value) => value; pick(...[get(Math.max, "constructor")])("return process")()',
    'const get = (target, key) => target[key]; for (const Build of Object.values({ build: get(Math.max, "constructor") })) Build("return process")()',
    'const get = (target, key) => target[key]; Object.entries({ build: get(Math.max, "constructor") })[0][1]("return process")()',
    'const get = (target, key) => target[key]; const values = [get(Math.max, "constructor")]; values.at(0)("return process")()',
    'const get = (target, key) => target[key]; const values = [get(Math.max, "constructor")]; values.pop()("return process")()',
    'const get = (target, key) => target[key]; const values = [get(Math.max, "constructor")]; for (const Build of values.values()) Build("return process")()',
    'const get = (target, key) => target[key]; const values = [get(Math.max, "constructor")]; for (const [, Build] of values.entries()) Build("return process")()',
    'const get = (target, key) => target[key]; const values = [get(Math.max, "constructor")]; values.some((Build) => Build("return process")())',
    'const get = (target, key) => target[key]; const values = [get(Math.max, "constructor")]; values.slice()[0]("return process")()',
    'const get = (target, key) => target[key]; const values = [get(Math.max, "constructor")]; values.map((value) => value)[0]("return process")()',
    'const get = (target, key) => target[key]; const values = [get(Math.max, "constructor")]; values.filter(() => true)[0]("return process")()',
    'const get = (target, key) => target[key]; const values = [get(Math.max, "constructor")]; values.concat([])[0]("return process")()',
    'const get = (target, key) => target[key]; const values = [[get(Math.max, "constructor")]]; values.flat()[0]("return process")()',
    'const get = (target, key) => target[key]; const values = [get(Math.max, "constructor")]; values.reduce((_, value) => value)("return process")()',
    'const get = (target, key) => target[key]; const values = [get(Math.max, "constructor")]; values.reverse()[0]("return process")()',
    'const get = (target, key) => target[key]; const values = [get(Math.max, "constructor")]; values.toSorted()[0]("return process")()',
    'const get = (target, key) => target[key]; const values = [get(Math.max, "constructor")]; values.splice(0, 1)[0]("return process")()',
    'const get = (target, key) => target[key]; const values = [get(Math.max, "constructor")]; values.values().next().value("return process")()',
    'const get = (target, key) => target[key]; const values = [get(Math.max, "constructor")]; const { pop: take } = values; take.call(values)("return process")()',
    'const get = (target, key) => target[key]; const values = [get(Math.max, "constructor")]; values.forEach((Build) => { const run = Build; run("return process")(); })',
    'const get = (target, key) => target[key]; const values = [get(Math.max, "constructor")]; const box = {}; values.some((Build) => { box.build = Build; return false; }); box.build("return process")()',
    'const get = (target, key) => target[key]; const pick = (_, value) => value; pick(...[0, get(Math.max, "constructor")])("return process")()',
    'const get = (target, key) => target[key]; const pick = (_, value) => value; const args = [0, get(Math.max, "constructor")]; pick(...args)("return process")()',
...
test('ordinary descriptor inspection and approved direct reflection remain available', () => {
  const safeSources = [
    ...
    'const get = (target, key) => target[key]; var Build = () => 7; try { throw 0; } catch (Build) { var Build = get(Math.max, "constructor"); } Build()',
    ...
    'for (const Build of [() => 7]) Build()',
    'let Build; for (Build of [() => 7]) Build()',
    'for (const [Build] of [[() => 7]]) Build()',
    'async function safe() { for await (const Build of [() => 7]) Build(); } safe()',
    'for (const Build of [...[() => 7]]) Build()',
    'const [Build] = [...[() => 7]]; Build()',
    'const pick = (value) => value; pick(...[() => 7])()',
    'for (const Build of Object.values({ build: () => 7 })) Build()',
    'Object.entries({ build: () => 7 })[0][1]()',
    'const cls = { constructor: { body: [] }, methods: new Map(), getters: new Map() }; (cls.constructor ? Boolean(cls.constructor.body) : false) || [...cls.methods.values(), ...cls.getters.values()].some((member) => Boolean(member.body))',
    'const values = [() => 7]; values.at(0)()',
    'const values = [() => 7]; values.pop()()',
    'const values = [() => 7]; for (const Build of values.values()) Build()',
    'const values = [() => 7]; for (const [, Build] of values.entries()) Build()',
