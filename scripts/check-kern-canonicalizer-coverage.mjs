...
import {
  ...
  m4108ParameterMigration,
} from './kern-canonicalizer/coverage-m4-108-parameter-migration.mjs';
import {
...
} from './kern-canonicalizer/runtime-cost-m4-98.mjs';
import {
...
if (process.argv.includes('--write')) {
  ...
  assertCoverageSummary(m4109ResidualAnalysisUrl, m4109ResidualAnalysis);
  assert.equal(
    m4109ResidualAnalysisHandoff.digest,
    'ad6240c77ed276d1f865beb702ceeb7c85767191dbaa3cf36f526505c4e555fb',
  );
  assert.equal(
    m4109ResidualAnalysisHandoff.inputCommit,
    '88e311ac5565ed424f71bb2f9ed7a18333a5e8e4',
  );
  assert.equal(m4109ResidualAnalysis.assignments.length, 15);
  assert.equal(
    m4109ResidualAnalysis.assignmentsDigest,
    'f200b876c0ed6dd9cd75cfebe1c46c3d6cf97b13e0422886bc13b0f02f46b203',
  );
  assert.equal(m4109ResidualAnalysis.frontier.evaluatedObservedSettings, 0);
  ...
  assert.equal(m497RuntimeCost.result.promotionBudgetDeficit, 3_934);
  assert.equal(m497RuntimeCost.optimization.parentFrameRestarts, 0);
  assert.equal(m497RuntimeCost.observations[1].loopIterations.rolledBack, 0);
  assert.deepEqual(m497RuntimeCost.promotion, {
    disposition: 'production-headroom-authenticated-promotion-budget-no-go',
    nextMilestone: 'M4.98',
    profilePromotionApproved: false,
  });
  assertCoverageSummary(m498RuntimeCostUrl, m498RuntimeCost);
  assert.equal(m498RuntimeCost.result.exactFloor, 46_381);
  assert.equal(m498RuntimeCost.result.floorReduction, 6_705);
  assert.equal(m498RuntimeCost.result.productionHeadroom, 19_155);
  assert.equal(m498RuntimeCost.result.promotionBudgetHeadroom, 2_771);
  assert.equal(m498RuntimeCost.optimization.propertyOrderAuthenticated, true);
  assert.equal(m498RuntimeCost.optimization.runtimeEngineChanged, false);
  assert.deepEqual(m498RuntimeCost.promotion, {
    disposition: 'promotion-budget-headroom-authenticated',
    nextMilestone: 'M4.99',
    profilePromotionApproved: false,
    promotionReady: true,
...
process.stdout.write(
  ...
  ` ${formatM496RuntimeBottleneckStatus(m496RuntimeBottleneck)}` +
  ` ${formatM497RuntimeCostStatus(m497RuntimeCost)}` +
