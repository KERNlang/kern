...
KIR_SHA256=${JSON.stringify(kirSha256)}; COMPILER_REQUEST_SHA256=${JSON.stringify(compilerRequestSha256)}; SEMANTIC_SHA256=${JSON.stringify(semanticSha256)}; ENTRY=${JSON.stringify(entry)}; MANIFEST_FILE=${JSON.stringify(manifestFile)}; ARTIFACT_PATH=${JSON.stringify(artifactPath)}; CAPABILITY_SEAL=${JSON.stringify(capabilitySeal)}; TARGET=${JSON.stringify(target)}; COUNT=${count}; LIMIT_KEYS=['maxBytes','maxCollectionLength','maxDepth','maxDiagnostics','maxEvents', 'maxIterations','maxStringBytes']
