...
const PUBLISHED_DIGEST = 'c24a3f59fab134a0845980550196f5d843c05d28986ea68a6e31642e3577dfdf';
const SOURCE_COMMIT = '828283e9694db3017dfc0121b6db8d6420f3988a';
const SUMMARY_URL = new URL('./coverage-prerequisite-m4-60.json', import.meta.url);

function fail(message) {
  throw new TypeError(`coverage M4.60 prerequisite rejection: ${message}`);
}
...
function publishedHandoff(value) {
  ...
  if (digest !== PUBLISHED_DIGEST) {
    fail('receipt must match the exact published M4.60 prerequisite');
  }
  return { digest, record: structuredClone(value), sourceCommit: SOURCE_COMMIT };
}

export function validatePublishedCanonicalizerPrerequisiteM460(value) {
  return publishedHandoff(value);
}

export function loadPublishedCanonicalizerPrerequisiteM460() {
