...
test('M4.84 argProvenanced has exact structural runtime floor 38773', () => {
  ...
  assert.equal(HEADROOM.promotion.disposition, 'approved');
  assert.equal(HEADROOM.promotion.nextMilestone, 'M4.85');
...
test('M4.84 keeps module-envelope admission outside the structural claim', () => {
  ...
  assert.equal(HEADROOM.witnesses.reduce((total, { parameterRows }) => total + parameterRows, 0), 19);
});
