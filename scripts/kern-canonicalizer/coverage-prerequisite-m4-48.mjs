...
const PUBLISHED_DIGEST = 'fbc4b671f665d1ed2ebb709201a4c3f4be27d9cec4f18708ce7130fd2b2a7b0a';
const SOURCE_COMMIT = 'c16ab453b49d850d58022160a577c23eb70a2142';
const SUMMARY_URL = new URL('./coverage-prerequisite-m4-48.json', import.meta.url);

function fail(message) {
  throw new TypeError(`coverage M4.48 prerequisite rejection: ${message}`);
}
...
function publishedHandoff(value) {
  ...
  if (digest !== PUBLISHED_DIGEST) fail('receipt must match the exact published M4.48 prerequisite');
  return { digest, record: structuredClone(value), sourceCommit: SOURCE_COMMIT };
}

export function validatePublishedCanonicalizerPrerequisiteM448(value) {
  return publishedHandoff(value);
}

export function loadPublishedCanonicalizerPrerequisiteM448() {
