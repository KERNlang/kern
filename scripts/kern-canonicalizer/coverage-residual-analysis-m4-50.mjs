...
const SUMMARY_URL = new URL('./coverage-residual-analysis-m4-50.json', import.meta.url);
const LIMIT_AXES = [
...
function fail(message) {
  throw new TypeError(`coverage M4.50 residual analysis rejection: ${message}`);
}
...
function residualState() {
  ...
  if (roots.size !== coverage.functions.length) {
    fail('source functions must match authenticated facts');
  }
  ...
  if (parameterReady.length !== 0) fail('M4.50 requires an empty parameter-ready partition');
  if (prerequisite.outcome !== 'bounded-exhaustion' || prerequisite.exhaustion === null) {
    fail('M4.50 requires authenticated bounded exhaustion');
  }
  if (prerequisite.parameterMigration.completeFunctions !== 0) {
    fail('M4.50 requires the authenticated parameter queue to be empty');
  }
  if (residual.length !== 39 || residual.length !== prerequisite.exhaustion.residualFunctionCount) {
    fail('residual facts must match the exact published M4.49 population');
  }
...
function buildCanonicalizerResidualAnalysisM450() {
...
export function validateCanonicalizerResidualAnalysisM450(value) {
  assertPlainReceiptData(value);
  const expected = buildCanonicalizerResidualAnalysisM450();
  if (JSON.stringify(value) !== JSON.stringify(expected)) {
    fail('analysis must match authenticated measurement');
  }
  return expected;
}

export function measureCanonicalizerResidualAnalysisM450() {
  return buildCanonicalizerResidualAnalysisM450();
}

export function writeCanonicalizerResidualAnalysisM450() {
  const result = buildCanonicalizerResidualAnalysisM450();
