...
const INPUT_COMMIT = 'e77fc4567543ad6984b86b97d8a7a8e469020ebd';
const RESIDUAL_ANALYSIS_DIGEST =
  'ad6240c77ed276d1f865beb702ceeb7c85767191dbaa3cf36f526505c4e555fb';
const SUMMARY_URL = new URL('./projection-analysis-m4-110.json', import.meta.url);
const LIMIT_AXES = [
...
function project(root, limits) {
  try {
    return { outcome: 'projected', profileRows: canonicalProfileRowsForFunction(root, limits) };
  } catch (error) {
    const projectionCode =
      typeof error?.code === 'string' && error.code.length > 0 ? error.code : 'projection-error';
...
function publishedHandoff(value) {
  ...
  if (digest !== PUBLISHED_DIGEST) fail('receipt must match the exact published M4.110 analysis');
  return { digest, inputCommit: INPUT_COMMIT, record: structuredClone(value) };
}

export function measureCanonicalizerProjectionAnalysisM4110() {
  const residualHandoff = loadPublishedCanonicalizerResidualAnalysisM4109();
  if (residualHandoff.digest !== RESIDUAL_ANALYSIS_DIGEST) fail('M4.109 input digest must remain exact');
  ...
  const requirements = legacyFacts.map((fact) => {
    ...
    if (probe.outcome !== 'projected') {
      return {
        id: fact.id,
        outcome: 'unsupported',
        parameterRows: residualHandoff.record.assignments
          .find(({ id }) => id === fact.id).parameterRows,
  ...
  const candidates = [...observedSettings.values()]
    .map((kirLimits) => {
      ...
      return {
        ...
        kirLimits,
        migratedParameterRows: witnesses.reduce((total, row) => total + row.parameterRows, 0),
  ...
  if (
    analysis.requirements.length !== 15 ||
    analysis.summary.projectedFunctions !== 13 ||
    analysis.summary.unsupportedFunctions !== 2 ||
    analysis.selectedNextAction?.kirLimits.maxDepth !== 76 ||
    analysis.selectedNextAction?.completeFunctions !== 9 ||
    analysis.selectedNextAction?.completeTools !== 4 ||
    analysis.selectedNextAction?.migratedParameterRows !== 134
  ) fail('measured M4.110 projection frontier must remain exact');
  assertPlainReceiptData(analysis);
  return analysis;
}

export function validatePublishedCanonicalizerProjectionAnalysisM4110(value) {
  return publishedHandoff(value);
}

export function loadPublishedCanonicalizerProjectionAnalysisM4110() {
  ...
  if (!source.equals(canonicalBytes(result.record))) fail('published receipt must use canonical JSON bytes');
  return result;
}
...
if (invokedPath !== undefined && realpathSync(invokedPath) === fileURLToPath(import.meta.url)) {
  ...
  writeCoverageSummary(SUMMARY_URL, measureCanonicalizerProjectionAnalysisM4110());
}
