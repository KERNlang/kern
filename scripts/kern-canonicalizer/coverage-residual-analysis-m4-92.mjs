...
const INPUT_COMMIT = '730aa181e1e3ea40b88dd22f74c58e853a706009';
const SUMMARY_URL = new URL('./coverage-residual-analysis-m4-92.json', import.meta.url);
...
function fail(message) {
  throw new TypeError(`coverage M4.92 residual analysis rejection: ${message}`);
}
...
function publishedHandoff(value) {
  ...
  if (digest !== PUBLISHED_DIGEST) fail('receipt must match the exact published M4.92 analysis');
...
function assertPublishedInput(receipt, canonicalizerPolicy) {
  ...
  if (JSON.stringify(actual) !== JSON.stringify(expected)) {
    fail('live semantic facts must match the exact published M4.91 input');
  }
}

export function measureCanonicalizerResidualAnalysisM492() {
  ...
  if (legacyFacts.length !== PUBLISHED_BASELINE.legacyParameterBlockers) {
    fail('legacy parameter population must remain exactly 18 functions');
  }
  ...
  if (parameterReady.length !== 0 || residual.length !== PUBLISHED_BASELINE.residualFunctionCount) {
    fail('M4.91 must expose an empty queue and exactly 18 residual functions');
  }
  ...
  if (assignmentsDigest !== EXPECTED_ASSIGNMENTS_DIGEST) {
    fail('reason assignments must reproduce the M4.91 exhaustion digest');
  }
...
export function validatePublishedCanonicalizerResidualAnalysisM492(value) {
  return publishedHandoff(value);
}

export function loadPublishedCanonicalizerResidualAnalysisM492() {
...
if (invokedPath !== undefined && realpathSync(invokedPath) === fileURLToPath(import.meta.url)) {
  ...
  writeCoverageSummary(SUMMARY_URL, measureCanonicalizerResidualAnalysisM492());
}
