...
test('M4.45 consumes exactly the frozen 388-row parameter queue', () => {
  ...
  assert.equal(prerequisite.parameterMigration.completeFunctions, 4);
  assert.equal(prerequisite.parameterMigration.completeTools, 3);
  assert.equal(prerequisite.parameterMigration.migratedParameterRows, 12);
...
test('M4.45 generated consumers reproduce only from repository writers', () => {
  ...
  assert.equal(sha256(checkerMain), '7adbe8a62c597fb42b6602d728e2c1aefb1dfc972bccafde3de6f50914363776');
  const built = createCanonicalizerComposition();
  const verified = verifyCanonicalizerComposition();
  assert.equal(built.compositeBytes.length, 49_409);
  assert.equal(
