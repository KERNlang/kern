...
const PUBLISHED_DIGEST = '9bba0c10b55e732392fa68dd7f7174135a4ff380875e15ea787e045b46d5610f';
const SOURCE_COMMIT = '9f60e3c3a43dd029626466223effbc08b51696b2';
const SUMMARY_URL = new URL('./coverage-prerequisite-m4-64.json', import.meta.url);

function fail(message) {
  throw new TypeError(`coverage M4.64 prerequisite rejection: ${message}`);
}
...
function publishedHandoff(value) {
  ...
  if (digest !== PUBLISHED_DIGEST) {
    fail('receipt must match the exact published M4.64 prerequisite');
  }
  return { digest, record: structuredClone(value), sourceCommit: SOURCE_COMMIT };
}

export function validatePublishedCanonicalizerPrerequisiteM464(value) {
  return publishedHandoff(value);
}

export function loadPublishedCanonicalizerPrerequisiteM464() {
