...
const PUBLISHED_DIGEST = 'a963c0df94b563eb7df5e50eba68faf12cd607f92229ab0c748c412eaa3e88ca';
const SOURCE_COMMIT = 'f198ec30b8b00c2cdb9aca2b9aeb7a2e38a5e1df';
const SUMMARY_URL = new URL('./coverage-prerequisite-m4-76.json', import.meta.url);

function fail(message) {
  throw new TypeError(`coverage M4.76 prerequisite rejection: ${message}`);
}
...
function publishedHandoff(value) {
  ...
  if (digest !== PUBLISHED_DIGEST) {
    fail('receipt must match the exact published M4.76 prerequisite');
  }
  return { digest, record: structuredClone(value), sourceCommit: SOURCE_COMMIT };
}

export function validatePublishedCanonicalizerPrerequisiteM476(value) {
  return publishedHandoff(value);
}

export function loadPublishedCanonicalizerPrerequisiteM476() {
