...
const INPUT_COMMIT = 'f36a870843ccdd222e8cf2e7595c0e205ed545bf';
const SUMMARY_URL = new URL('./coverage-residual-analysis-m4-62.json', import.meta.url);
...
function fail(message) {
  throw new TypeError(`coverage M4.62 residual analysis rejection: ${message}`);
}
...
function publishedHandoff(value) {
  ...
  if (digest !== PUBLISHED_DIGEST) fail('receipt must match the exact published M4.62 analysis');
...
function assertPublishedInput(receipt, canonicalizerPolicy) {
  ...
  if (JSON.stringify(actual) !== JSON.stringify(expected)) {
    fail('live semantic facts must match the exact published M4.61 input');
  }
}

export function measureCanonicalizerResidualAnalysisM462() {
  ...
  if (legacyFacts.length !== PUBLISHED_BASELINE.legacyParameterBlockers) {
    fail('legacy parameter population must remain exactly 30 functions');
  }
  ...
  if (parameterReady.length !== 0 || residual.length !== PUBLISHED_BASELINE.residualFunctionCount) {
    fail('M4.61 must expose an empty queue and exactly 30 residual functions');
  }
  ...
  if (assignmentsDigest !== EXPECTED_ASSIGNMENTS_DIGEST) {
    fail('reason assignments must reproduce the M4.61 exhaustion digest');
  }
...
export function validatePublishedCanonicalizerResidualAnalysisM462(value) {
  return publishedHandoff(value);
}

export function loadPublishedCanonicalizerResidualAnalysisM462() {
...
if (invokedPath !== undefined && realpathSync(invokedPath) === fileURLToPath(import.meta.url)) {
  ...
  writeCoverageSummary(SUMMARY_URL, measureCanonicalizerResidualAnalysisM462());
}
