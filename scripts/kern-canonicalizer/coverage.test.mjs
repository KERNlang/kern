...
test('the handwritten corpus produces one deterministic catalog-bound selection receipt', () => {
  ...
  assert.equal(first.functions.length, 106);
  assert.deepEqual(
  ...
  assert.equal(first.functions.filter(({ excludedProperties }) => excludedProperties.includes('fn.params')).length, 18);
  assert.equal(first.baseCompleteFunctions, 88);
...
test('the active profile rejects rows above the M4.90 node, property, and value ceilings', () => {
  const parsed = parseDocumentWithDiagnostics([
  ...
  const blockers = profileBlockersForFunction(
    ...
    { nodes: 75, properties: 78, values: 581 },
  );
...
test('profile node ceilings use the codec-measured row count', () => {
  ...
  const blockers = profileBlockersForFunction(
    ...
    loadCoveragePolicy().base,
    loadCanonicalizerPolicy().profileLimits,
    { nodes: 75, properties: 3, values: 4 },
  );
