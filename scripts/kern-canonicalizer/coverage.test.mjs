...
test('the handwritten corpus produces one deterministic catalog-bound selection receipt', () => {
  ...
  assert.equal(first.functions.length, 105);
  assert.deepEqual(
  ...
  assert.equal(first.functions.filter(({ excludedProperties }) => excludedProperties.includes('fn.params')).length, 23);
  assert.equal(first.baseCompleteFunctions, 81);
...
test('the active profile rejects rows above the M4.76 node, property, and value ceilings', () => {
  const parsed = parseDocumentWithDiagnostics([
  ...
  const blockers = profileBlockersForFunction(
    ...
    { nodes: 39, properties: 54, values: 462 },
  );
...
test('profile node ceilings use the codec-measured row count', () => {
  ...
  const blockers = profileBlockersForFunction(
    ...
    loadCoveragePolicy().base,
    loadCanonicalizerPolicy().profileLimits,
    { nodes: 39, properties: 3, values: 4 },
  );
