...
test('the handwritten corpus produces one deterministic catalog-bound selection receipt', () => {
  ...
  assert.equal(first.functions.filter(({ excludedProperties }) => excludedProperties.includes('fn.params')).length, 4);
  assert.equal(first.baseCompleteFunctions, 103);
...
test('the active profile rejects rows above the M4.118 node, property, and value ceilings', () => {
  const parsed = parseDocumentWithDiagnostics([
  ...
  const blockers = profileBlockersForFunction(
    ...
    { nodes: 123, properties: 194, values: 2412 },
  );
...
test('profile node ceilings use the codec-measured row count', () => {
  ...
  const blockers = profileBlockersForFunction(
    ...
    loadCoveragePolicy().base,
    loadCanonicalizerPolicy().profileLimits,
    { nodes: 123, properties: 3, values: 4 },
  );
