...
test('the handwritten corpus produces one deterministic catalog-bound selection receipt', () => {
  ...
  assert.equal(first.functions.filter(({ excludedProperties }) => excludedProperties.includes('fn.params')).length, 30);
  assert.equal(first.baseCompleteFunctions, 73);
...
test('the active profile rejects rows above the M4.64 node and M4.56 property ceilings', () => {
  const parsed = parseDocumentWithDiagnostics([
  ...
  const blockers = profileBlockersForFunction(
    ...
    { nodes: 29, properties: 51, values: 73 },
  );
...
test('profile node ceilings use the codec-measured row count', () => {
  ...
  const blockers = profileBlockersForFunction(
    ...
    loadCoveragePolicy().base,
    loadCanonicalizerPolicy().profileLimits,
    { nodes: 29, properties: 3, values: 4 },
  );
