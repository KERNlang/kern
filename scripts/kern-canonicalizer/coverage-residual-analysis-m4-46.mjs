...
function fail(message) {
  throw new TypeError(`coverage M4.46 residual analysis rejection: ${message}`);
}
...
function residualState() {
  ...
  if (parameterReady.length !== 0) fail('M4.46 requires an empty parameter-ready partition');
  if (prerequisite.outcome !== 'bounded-exhaustion' || prerequisite.exhaustion === null) {
    fail('M4.46 requires authenticated bounded exhaustion');
  }
  if (prerequisite.parameterMigration.completeFunctions !== 0) {
    fail('M4.46 requires the authenticated parameter queue to be empty');
  }
  if (residual.length !== 43 || residual.length !== prerequisite.exhaustion.residualFunctionCount) {
    fail('residual facts must match the exact published M4.45 population');
  }
...
function buildCanonicalizerResidualAnalysisM446() {
...
export function validateCanonicalizerResidualAnalysisM446(value) {
  assertPlainReceiptData(value);
  const expected = buildCanonicalizerResidualAnalysisM446();
...
export function measureCanonicalizerResidualAnalysisM446() {
  return buildCanonicalizerResidualAnalysisM446();
}
