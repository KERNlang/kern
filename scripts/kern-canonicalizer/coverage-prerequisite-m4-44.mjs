...
const PUBLISHED_DIGEST = '9741650d8567016fb029a8e51b4706da1da131d9870c94a3221b4550792dee01';
const SOURCE_COMMIT = 'dd977ff493250127e2e416ffb4e3ab68985a61dc';
const SUMMARY_URL = new URL('./coverage-prerequisite-m4-44.json', import.meta.url);

function fail(message) {
  throw new TypeError(`coverage M4.44 prerequisite rejection: ${message}`);
}
...
function publishedHandoff(value) {
  ...
  if (digest !== PUBLISHED_DIGEST) fail('receipt must match the exact published M4.44 prerequisite');
  return { digest, record: structuredClone(value), sourceCommit: SOURCE_COMMIT };
}

export function validatePublishedCanonicalizerPrerequisiteM444(value) {
  return publishedHandoff(value);
}

export function loadPublishedCanonicalizerPrerequisiteM444() {
