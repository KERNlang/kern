...
export function measureCanonicalizerRuntimeCostM4104(iterationBudget) {
  if (!Number.isSafeInteger(iterationBudget) || iterationBudget <= 0) {
    throw new TypeError('M4.104 iteration budget must be a positive safe integer');
  }
  ...
  if ('format' in linked) throw new Error('M4.104 candidate failed to link');
  const arguments_ = [
...
if (invokedPath !== undefined && resolve(invokedPath) === fileURLToPath(import.meta.url)) {
  ...
  if (Number.isSafeInteger(budget) && budget > 0) {
    process.stdout.write(
      `${JSON.stringify(measureCanonicalizerRuntimeCostM4104(budget), null, 2)}\n`,
    );
  }
}
