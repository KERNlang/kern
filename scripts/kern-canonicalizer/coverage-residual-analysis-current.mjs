...
function fail(message) {
  throw new TypeError(`coverage current residual analysis rejection: ${message}`);
}
...
function residualState() {
  ...
  if (parameterReady.length !== 0) fail('current analysis requires an empty parameter-ready partition');
  if (prerequisite.outcome !== 'bounded-exhaustion' || prerequisite.exhaustion === null) {
    fail('current analysis requires authenticated bounded exhaustion');
  }
  if (prerequisite.parameterMigration.completeFunctions !== 0) {
    fail('current analysis requires the authenticated parameter queue to be empty');
  }
  if (residual.length !== prerequisite.exhaustion.residualFunctionCount) {
    fail('residual facts must match the live prerequisite population');
  }
...
export function measureCurrentCanonicalizerResidualAnalysis() {
  ...
  if (assignmentsDigest !== prerequisite.exhaustion.reasonAssignmentsDigest) {
    fail('assignment digest must reproduce the live exhaustion receipt');
  }
