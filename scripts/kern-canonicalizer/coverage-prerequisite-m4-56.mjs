...
const PUBLISHED_DIGEST = '13a420892453e03eed314ddad2f50ceeed4fe0f01e50cc3ee1a72a253caad26b';
const SOURCE_COMMIT = '8928684827706b2abac1f4906f785a389afb91c6';
const SUMMARY_URL = new URL('./coverage-prerequisite-m4-56.json', import.meta.url);

function fail(message) {
  throw new TypeError(`coverage M4.56 prerequisite rejection: ${message}`);
}
...
function publishedHandoff(value) {
  ...
  if (digest !== PUBLISHED_DIGEST) {
    fail('receipt must match the exact published M4.56 prerequisite');
  }
  return { digest, record: structuredClone(value), sourceCommit: SOURCE_COMMIT };
}

export function validatePublishedCanonicalizerPrerequisiteM456(value) {
  return publishedHandoff(value);
}

export function loadPublishedCanonicalizerPrerequisiteM456() {
