...
const INPUT_COMMIT = '1fe7851101cf2a25e1aebfd561655bb458aec66b';
const SUMMARY_URL = new URL('./coverage-residual-analysis-m4-74.json', import.meta.url);
...
function fail(message) {
  throw new TypeError(`coverage M4.74 residual analysis rejection: ${message}`);
}
...
function publishedHandoff(value) {
  ...
  if (digest !== PUBLISHED_DIGEST) fail('receipt must match the exact published M4.74 analysis');
...
function assertPublishedInput(receipt, canonicalizerPolicy) {
  ...
  if (JSON.stringify(actual) !== JSON.stringify(expected)) {
    fail('live semantic facts must match the exact published M4.73 input');
  }
}

export function measureCanonicalizerResidualAnalysisM474() {
  ...
  if (legacyFacts.length !== PUBLISHED_BASELINE.legacyParameterBlockers) {
    fail('legacy parameter population must remain exactly 24 functions');
  }
  ...
  if (parameterReady.length !== 0 || residual.length !== PUBLISHED_BASELINE.residualFunctionCount) {
    fail('M4.73 must expose an empty queue and exactly 24 residual functions');
  }
  ...
  if (assignmentsDigest !== EXPECTED_ASSIGNMENTS_DIGEST) {
    fail('reason assignments must reproduce the M4.73 exhaustion digest');
  }
...
export function validatePublishedCanonicalizerResidualAnalysisM474(value) {
  return publishedHandoff(value);
}

export function loadPublishedCanonicalizerResidualAnalysisM474() {
...
if (invokedPath !== undefined && realpathSync(invokedPath) === fileURLToPath(import.meta.url)) {
  ...
  writeCoverageSummary(SUMMARY_URL, measureCanonicalizerResidualAnalysisM474());
}
