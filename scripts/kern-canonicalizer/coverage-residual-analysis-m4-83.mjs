...
const INPUT_COMMIT = '89083ba126201067c918ea7e130382ca171f4097';
const SUMMARY_URL = new URL('./coverage-residual-analysis-m4-83.json', import.meta.url);
...
function fail(message) {
  throw new TypeError(`coverage M4.83 residual analysis rejection: ${message}`);
}
...
function publishedHandoff(value) {
  ...
  if (digest !== PUBLISHED_DIGEST) fail('receipt must match the exact published M4.83 analysis');
...
function assertPublishedInput(receipt, canonicalizerPolicy) {
  ...
  if (JSON.stringify(actual) !== JSON.stringify(expected)) {
    fail('live semantic facts must match the exact published M4.82 input');
  }
}

export function measureCanonicalizerResidualAnalysisM483() {
  ...
  if (legacyFacts.length !== PUBLISHED_BASELINE.legacyParameterBlockers) {
    fail('legacy parameter population must remain exactly 22 functions');
  }
  ...
  if (parameterReady.length !== 0 || residual.length !== PUBLISHED_BASELINE.residualFunctionCount) {
    fail('M4.82 must expose an empty queue and exactly 22 residual functions');
  }
  ...
  if (assignmentsDigest !== EXPECTED_ASSIGNMENTS_DIGEST) {
    fail('reason assignments must reproduce the M4.82 exhaustion digest');
  }
...
export function validatePublishedCanonicalizerResidualAnalysisM483(value) {
  return publishedHandoff(value);
}

export function loadPublishedCanonicalizerResidualAnalysisM483() {
...
if (invokedPath !== undefined && realpathSync(invokedPath) === fileURLToPath(import.meta.url)) {
  ...
  writeCoverageSummary(SUMMARY_URL, measureCanonicalizerResidualAnalysisM483());
}
