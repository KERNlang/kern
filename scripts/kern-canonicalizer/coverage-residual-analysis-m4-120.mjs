...
const INPUT_COMMIT = '2ffe06f0c31e7b6cbdea62f47df97f5a94b66dad';
const SUMMARY_URL = new URL('./coverage-residual-analysis-m4-120.json', import.meta.url);
...
function fail(message) {
  throw new TypeError(`coverage M4.120 residual analysis rejection: ${message}`);
}
...
function publishedHandoff(value) {
  ...
  if (digest !== PUBLISHED_DIGEST) fail('receipt must match the exact published M4.120 analysis');
...
function assertPublishedInput(receipt, canonicalizerPolicy, coveragePolicyDigest) {
  ...
  if (JSON.stringify(actual) !== JSON.stringify(expected)) {
    fail('live semantic facts must match the exact published M4.119 input');
  }
}
...
export function measureCanonicalizerResidualAnalysisM4120() {
  ...
  if (legacyFacts.length !== PUBLISHED_BASELINE.legacyParameterBlockers) {
    fail('legacy parameter population must remain exactly 5 functions');
  }
  ...
  if (parameterReady.length !== 0 || residual.length !== PUBLISHED_BASELINE.residualFunctionCount) {
    fail('M4.119 must expose an empty queue and exactly 5 residual functions');
  }
  ...
  if (assignmentsDigest !== EXPECTED_ASSIGNMENTS_DIGEST) {
    fail('reason assignments must reproduce the M4.119 exhaustion frontier');
  }
  ...
  if (
    ...
    fail('M4.119 residual frontier must expose no actionable profile widening');
  }
  assertPlainReceiptData(analysis);
  return analysis;
}

export function validatePublishedCanonicalizerResidualAnalysisM4120(value) {
  return publishedHandoff(value);
}

export function loadPublishedCanonicalizerResidualAnalysisM4120() {
...
if (invokedPath !== undefined && realpathSync(invokedPath) === fileURLToPath(import.meta.url)) {
  ...
  writeCoverageSummary(SUMMARY_URL, measureCanonicalizerResidualAnalysisM4120());
}
