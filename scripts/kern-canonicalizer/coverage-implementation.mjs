...
export function measureCanonicalizerCoverage(policyInput, canonicalizerPolicyInput, {
  ...
  const corpusPaths = new Set(policy.corpus.map(({ path }) => path));
  if ([...sourceOverrides.keys()].some((path) => !corpusPaths.has(path))) {
    fail('source overrides must name only declared corpus members');
  }
  ...
  for (const member of policy.corpus) {
    ...
    if (override !== undefined && !(override instanceof Uint8Array)) {
      fail(`source override ${member.path} must be bytes`);
    }
    const sourceBytes = override === undefined
    ...
    const source = sourceBytes.toString('utf8');
    if (/^\ufeff?[ \t]*#\s*(?:GENERATED FILE\b|@generated by kern\b)/imu.test(source)) {
      fail(`corpus member ${member.path} is generated`);
    }
