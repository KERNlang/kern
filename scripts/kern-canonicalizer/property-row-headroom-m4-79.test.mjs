...
import {
  loadCanonicalizerPropertyRowHeadroomM479,
  measureCanonicalizerPropertyRowHeadroomM479,
  validateCanonicalizerPropertyRowHeadroomM479,
} from './property-row-headroom-m4-79.mjs';

const summaryUrl = new URL('./property-row-headroom-m4-79.json', import.meta.url);
const RECEIPT_DIGEST = 'd8683f1440e8bb0f8496ab1845c83c7dabe73dbfd26114b78685d8c8e1cf830b';

test('M4.79 freezes the exact property-row structural runtime NO-GO receipt', () => {
  ...
  assert.deepEqual(measureCanonicalizerPropertyRowHeadroomM479(), receipt);
...
test('M4.79 rejects receipt drift, decorated data, cycles, and shared references', () => {
  const actual = loadCanonicalizerPropertyRowHeadroomM479();
  for (const mutate of [
    (copy) => { copy.format = 'kern.kir-canonicalizer.property-row-headroom.3'; },
    ...
    mutate(copy);
    assert.throws(
      () => validateCanonicalizerPropertyRowHeadroomM479(copy),
      /coverage M4\.79 property-row headroom rejection/u,
    );
  }
  const decorated = Object.assign(Object.create({ inherited: true }), actual);
  assert.throws(
    () => validateCanonicalizerPropertyRowHeadroomM479(decorated),
    /coverage M4\.79 property-row headroom rejection/u,
  );
  const shared = structuredClone(actual);
  shared.future = shared.limits;
  assert.throws(
    () => validateCanonicalizerPropertyRowHeadroomM479(shared),
    /cycles or shared references/u,
  );
  const cyclic = structuredClone(actual);
  cyclic.future = cyclic;
  assert.throws(
    () => validateCanonicalizerPropertyRowHeadroomM479(cyclic),
    /cycles or shared references/u,
  );
});

test('M4.79 preserves M4.78 and reproduces in a fresh locale-independent process', () => {
  ...
  const fresh = spawnSync(process.execPath, [
    '--input-type=module',
    '-e',
    "import {loadCanonicalizerPropertyRowHeadroomM479 as load} from './scripts/kern-canonicalizer/property-row-headroom-m4-79.mjs'; process.stdout.write(JSON.stringify(load()))",
  ], {
  ...
  assert.deepEqual(JSON.parse(fresh.stdout), loadCanonicalizerPropertyRowHeadroomM479());
});
