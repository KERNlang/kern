...
const PUBLISHED_DIGEST = '220becc58afa59bb35f1fef2246038d7c7763b49db65d615f6c5725c87659c76';
const SOURCE_COMMIT = '99905b044c3d981998a3beef846da283dac4a94c';
const SUMMARY_URL = new URL('./coverage-prerequisite-m4-52.json', import.meta.url);

function fail(message) {
  throw new TypeError(`coverage M4.52 prerequisite rejection: ${message}`);
}
...
function assertPlainReceiptData(value, seen = new Set()) {
  ...
  if (seen.has(value)) fail('receipt data must not contain cycles');
  seen.add(value);
...
function publishedHandoff(value) {
  ...
  if (digest !== PUBLISHED_DIGEST) fail('receipt must match the exact published M4.52 prerequisite');
  return { digest, record: structuredClone(value), sourceCommit: SOURCE_COMMIT };
}

export function validatePublishedCanonicalizerPrerequisiteM452(value) {
  return publishedHandoff(value);
}

export function loadPublishedCanonicalizerPrerequisiteM452() {
