...
const INPUT_COMMIT = '2cb03f0e84f6c586dd28404d331a67dd2bb839bb';
const SUMMARY_URL = new URL('./coverage-residual-analysis-m4-114.json', import.meta.url);
...
function fail(message) {
  throw new TypeError(`coverage M4.114 residual analysis rejection: ${message}`);
}
...
function publishedHandoff(value) {
  ...
  if (digest !== PUBLISHED_DIGEST) fail('receipt must match the exact published M4.114 analysis');
...
function assertPublishedInput(receipt, canonicalizerPolicy) {
  ...
  // pre-analysis digest remains part of the published baseline.
  const actual = {
  ...
  if (JSON.stringify(actual) !== JSON.stringify(expected)) {
    fail('live semantic facts must match the exact published M4.113 input');
  }
}

export function measureCanonicalizerResidualAnalysisM4114() {
  ...
  if (legacyFacts.length !== PUBLISHED_BASELINE.legacyParameterBlockers) {
    fail('legacy parameter population must remain exactly 6 functions');
  }
  ...
  if (parameterReady.length !== 0 || residual.length !== PUBLISHED_BASELINE.residualFunctionCount) {
    fail('M4.113 must expose an empty queue and exactly 6 residual functions');
  }
  ...
  if (assignmentsDigest !== EXPECTED_ASSIGNMENTS_DIGEST) {
    fail('reason assignments must reproduce the M4.113 exhaustion frontier');
  }
  ...
  if (
    ...
    analysis.frontier.evaluatedObservedSettings !== 1 ||
    analysis.frontier.actionableCandidates.length !== 1 ||
    analysis.selectedNextAction === null
  ) {
    fail('M4.113 residual frontier must expose exactly one actionable profile widening');
  }
  assertPlainReceiptData(analysis);
  return analysis;
}

export function validatePublishedCanonicalizerResidualAnalysisM4114(value) {
  return publishedHandoff(value);
}

export function loadPublishedCanonicalizerResidualAnalysisM4114() {
...
if (invokedPath !== undefined && realpathSync(invokedPath) === fileURLToPath(import.meta.url)) {
  ...
  writeCoverageSummary(SUMMARY_URL, measureCanonicalizerResidualAnalysisM4114());
}
