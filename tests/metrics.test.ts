...
describe('Metrics Engine', () => {
  ...
  describe('collectLanguageMetrics — dashboard.kern', () => {
    test('produces valid metrics', async () => {
      ...
      const { collectLanguageMetrics } = await import(resolve(ROOT, 'src/metrics.ts'));
      ...
      expect(metrics.themeRefCount).toBeGreaterThan(0); // $bar refs
      expect(metrics.styleMetrics.escapeRatio).toBe(0); // all mapped styles
      expect(metrics.tokenEfficiency).toBeNull(); // no result passed
    });
  });
