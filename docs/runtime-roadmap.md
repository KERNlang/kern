...
- `executeKernSourceAsync`, a browser-safe async preview that delegates
  ...
  providers in straight-line code, the matched arm of `if`/`else`, and
  sequential `for` / `each` loop bodies
...
- `kern run --capabilities` reports async-planned requirements, missing async
  ...
  `while`, `try`, `branch`, helper functions, and streams are reported
  separately from missing provider flags
...
`while`, `branch`, `try`, and helper functions, and broad async control flow.
...
3. **Capability boundary**
   ...
   in straight-line statements, the matched arm of `if`/`else`, and sequential
   `for` / `each` loop bodies. The Node CLI
   ...
   `asyncBoundaryRequired`; `while`, `branch`, `try`, helper functions, streams,
   and broad async control-flow execution are still future work in the CLI.
...
| `fs.readText` / `fs.writeText` / `fs.list` | Planned | Async planned | Must be host-injected; preview-runnable in `executeKernSourceAsync` straight-line / matched `if` arm / sequential `for` and `each` loop bodies; Node CLI preview provides root-scoped `fs.list` / `fs.readText` and opt-in `fs.writeText` |
| `net.fetch` | Planned | Async planned | Must be host-injected; preview-runnable in `executeKernSourceAsync` straight-line / matched `if` arm / sequential `for` and `each` loop bodies; Node CLI preview requires explicit `--allow-net <origin>` or `--allow-net data:` and denies redirects |
| `llm.complete` | Planned | Async planned | Must be host-injected; preview-runnable in `executeKernSourceAsync` straight-line / matched `if` arm / sequential `for` and `each` loop bodies; Node CLI preview provides deterministic `--llm-response <text>` and OpenAI-compatible `--llm-provider openai` |
...
inside `while`, `branch`, `try`, helper functions, streams, and broader control
...
- `unsupportedAsyncExecutions`: async-planned capability calls present in source
  locations the narrow async preview executor still rejects, such as `while`,
  `branch`, `try`, streams, or helper functions outside `main`.
...
- Expand the narrow async source preview into full async control-flow execution:
  `while`, `branch`, `try`/`finally`, helper functions, and streams.
