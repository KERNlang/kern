...
export function AudioSettings() {
  ...
  return (
    <SettingsSection
      title={t('settings.audio.title', 'Audio & Analysis')}
      description={t('settings.audio.description', 'Configure analysis sensitivity and audio processing defaults.')}
    >
      <div className="space-y-8">
        <div>
          <h3 className="text-sm font-medium text-white mb-4">
            {t('settings.masteringDefaults.title', 'Mastering Defaults')}
          </h3>
          <p className="text-xs text-zinc-500 mb-3">{t('settings.masteringDefaults.description', 'Set your preferred starting point for the mastering chain.')}</p>
          <MasteringDefaultsSection />
        </div>
        <div className="bg-zinc-800 h-px" />
        <div>
          <h3 className="text-sm font-medium text-white mb-4">
            {t('settings.audioDevice.title', 'Audio Output')}
          </h3>
          <AudioDeviceSelector value={settings.audioOutputDeviceId} onChange={setAudioOutputDeviceId} />
        </div>
        <div>
          <h3 className="text-sm font-medium text-white mb-4">
            {t('settings.exportDefaults.title', 'Export Defaults')}
          </h3>
          <p className="text-xs text-zinc-500 mb-3">{t('settings.exportDefaultsDescription', 'Set your preferred export format.')}</p>
          <ExportDefaultsSection isPro={isPro} exportFormat={exportFormat} mp3Bitrate={mp3Bitrate} />
        </div>
        <div className="bg-zinc-800 h-px" />
        <div>
          <h3 className="text-sm font-medium text-white mb-4">
            {t('settings.audioTab.defaultSensitivity.title', 'Default Analysis Sensitivity')}
          </h3>
          <div className="bg-zinc-900 rounded-lg p-4 border border-zinc-800">
            <PresetSelector value={presetId} onChange={selectPreset} isPro={isPro} />
        ...
        <div>
          <div className="flex items-center gap-2 mb-4">
            <h3 className="text-sm font-medium text-white">
              {t('settings.audioTab.fixStrength.title', 'Fix Strength')}
            </h3>
            <div className="relative group">
              ...
              <div className="absolute left-0 bottom-full mb-2 hidden group-hover:block z-50 w-72 p-3 bg-zinc-900 border border-zinc-700 rounded-lg shadow-xl text-xs text-zinc-300">
                {t('settings.audioTab.fixStrength.tooltip', "fixStrengthNote")}
              </div>
            </div>
          </div>
          <div className="bg-zinc-900 rounded-lg p-4 border border-zinc-800">
        ...
        <div>
          <div className="flex items-center gap-2 mb-4">
            <h3 className="text-sm font-medium text-white">
              {t('settings.audioTab.stemPresence.title', 'Stem Presence Threshold')}
            </h3>
            <div className="relative group">
              <Icon name="infoCircle" size="sm" className="text-zinc-500 hover:text-orange-500 cursor-help transition-colors" />
              <div className="absolute left-0 bottom-full mb-2 hidden group-hover:block z-50 w-72 p-3 bg-zinc-900 border border-zinc-700 rounded-lg shadow-xl text-xs text-zinc-300">
                {t('settings.audioTab.stemPresence.tooltip', "stemPresenceTooltip")}
              </div>
            </div>
          </div>
          <div className="bg-zinc-900 rounded-lg p-4 border border-zinc-800">
            ...
            <div className="bg-zinc-950 rounded-lg p-3 mt-4 border border-zinc-800">
        ...
        <div>
          <div className="flex items-center gap-2 mb-4">
            <h3 className="text-sm font-medium text-white">
              {t('settings.audioTab.perStem.title', 'Per-Stem Detection Thresholds')}
            </h3>
            <div className="relative group">
              <Icon name="infoCircle" size="sm" className="text-zinc-500 hover:text-orange-500 cursor-help transition-colors" />
              <div className="absolute left-0 bottom-full mb-2 hidden group-hover:block z-50 w-72 p-3 bg-zinc-900 border border-zinc-700 rounded-lg shadow-xl text-xs text-zinc-300">
                {t('settings.audioTab.perStem.tooltip', "perStemTooltip")}
              </div>
            </div>
          </div>
          <div className="bg-zinc-900 rounded-lg p-4 border border-zinc-800">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <StemThresholdSlider stem={"drums"} value={perStemThresholds.drums} onChange={setDrums} defaultValue={"-30"} disabled={!isPro} />
              <StemThresholdSlider stem={"bass"} value={perStemThresholds.bass} onChange={setBass} defaultValue={"-35"} disabled={!isPro} />
              <StemThresholdSlider stem={"vocals"} value={perStemThresholds.vocals} onChange={setVocals} defaultValue={"-30"} disabled={!isPro} />
              <StemThresholdSlider stem={"guitar"} value={perStemThresholds.guitar} onChange={setGuitar} defaultValue={"-45"} disabled={!isPro} />
              <StemThresholdSlider stem={"piano"} value={perStemThresholds.piano} onChange={setPiano} defaultValue={"-45"} disabled={!isPro} />
              <StemThresholdSlider stem={"other"} value={perStemThresholds.other} onChange={setOther} defaultValue={"-50"} disabled={!isPro} />
            </div>
        ...
        {isPro && (
          <>
            <div>
              <h3 className="text-sm font-medium text-white mb-4">
                {t('settings.audioTab.customThresholds.title', 'Custom Thresholds Configuration')}
              </h3>
              <div className="bg-zinc-900 rounded-lg p-6 border border-zinc-800">
                ...
                <ToleranceSliders value={customThresholds} onChange={setThreshold} disabled={!isPro} />
              </div>
            </div>
          </>
        )}
        ...
        <div>
          <h3 className="text-sm font-medium text-white mb-4">
            {t('settings.audioTab.signalProcessing.title', 'Signal Processing')}
          </h3>
          <div className="bg-zinc-900 rounded-lg p-4 border border-zinc-800">
