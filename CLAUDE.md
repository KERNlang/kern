...
- **Rule-coverage gate + finding-message voice live in [CONTRIBUTING.md](CONTRIBUTING.md) under "Adding a review rule".** CI runs `pnpm check:rule-coverage` (`scripts/check-rule-coverage.mjs`), which fails if a rule ID in the REGISTRY has no quoted reference in `packages/review/tests/` or `packages/review-mcp/tests/`. New rules need a test in the same PR. Voice essentials: code is the subject (not the developer), present tense, verb-first headline ≤ 90 chars, suggestion ≤ 200 chars, no trailing period, no emoji (plain Unicode glyphs OK in CLI scaffolding but never inside the message body). The `did you mean 'X'?` suggestion pattern is fine — it matches TypeScript/rustc.
