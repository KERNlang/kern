<div align="center">
  ...
  **Built for humans and AI.** 192-line spec. 13 compile targets. 206 review rules.<br>
...
kern review src/ --recursive                                  # Static analysis (206 rules, taint tracking)
...
Write `.kern` once, compile to 13 targets. Or skip `.kern` entirely and use `kern review` to scan your existing TypeScript and Python for security bugs, unguarded effects, flaky tests, and prompt injection — 206 AST-based rules that catch what ESLint misses.
...
**206 rules** across base, security, framework, performance, test-quality, null-safety, dead-logic, concept, and taint-aware analysis layers.

Recent coverage includes broadly reusable Next.js, Storybook, and Playwright checks: App Router body parsing, cache and header hazards, client-boundary env exposure, cookie hardening, SWR invalidation drift, focused tests, fixed Playwright sleeps, brittle `networkidle` waits, non-deterministic stories, and unmocked Storybook network calls.
...
| **[@kernlang/review](https://www.npmjs.com/package/@kernlang/review)** | 206 rules, taint tracking, OWASP LLM01, concept model |
