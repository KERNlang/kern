...
| **RT12J-TD16** | `ledger-schema.test.mjs` needs no edit because its row assertions use only synthetic documents | Its `the parity ledger carries the first deferred node kind` row (`ledger-schema.test.mjs:54-66`) deep-equals the checked-in ledger's single `while` row; the sequential parity-ledger gate failed there immediately after the two jump rows landed | Update that exact-content assertion to the sorted three-row document. The schema and its synthetic row-count assertions remain unchanged |

### Review round: RT12J-D1's frame search, 5 reviewers, 2026-09-08 (review-1788834788508-kpf3jc-kern5-rt12-jumps)

| ID | Reviewer finding | Fix | Oracle |
| --- | --- | --- | --- |
| **RT12J-TD17** | All five reviewers of `2c6f4abd` flagged `expression.ts:227-240`'s frame search as fail-open. The `continue` arm's `while (frames[frames.length - 1]?.loop === undefined) frames.pop();` reads `frames[-1]` as `undefined` once the stack is empty, so the predicate never turns `false` and `pop()` on an empty array is a no-op: an infinite loop that calls neither `meter.step()` nor `checkAbort()`, so it is invisible to `maxSteps` and to cancellation. The `break` arm's twin defect pops every frame looking for one with a `loop`; finding none, it empties the stack and the walk falls out of `while (frames.length > 0)` and returns `{ kind: 'drained' }` — the entry policy's fail-open completion, not the mandatory return the handler never reached. Both are unreachable through a compiled program (`link.ts`'s `loopDepth` gate refuses an unlabelled jump outside a loop before linking), but `walkStatements` is exported and this suite's own oracles above drive it directly with hand-built statements | Both arms now run a bounded, non-mutating scan (`depth` from `frames.length - 1` down to `0`) for the nearest frame with `loop !== undefined` *before* touching `frames`. If none exists, both raise `KernKirFault('unsupported-runtime-input', 'execution', 'KIR_JUMP_WITHOUT_LOOP_FRAME')` with no frame popped. The two RT-1 `checkAbort()` sites are untouched (still exactly two, per `tick-discipline.test.mjs` in this slice and in RT10-pre), and the fault path adds no `meter.step()` beyond what every other RT-1 fault path already skips |
| — | — | — | `walker-coverage.test.mjs` gained three rows: (1) a bare `continue` with no loop frame — driven in a throwaway child process under a hard OS-level timeout, since the unfixed loop has no yield point an in-process `Promise.race` or step budget could bound — asserts the fault fires with the `KIR_JUMP_WITHOUT_LOOP_FRAME` label rather than the process hanging; (2) a bare `break` followed by a `return` asserts the same fault fires instead of the handler draining past its own return; (3) a `continue`/`break` inside an `if` frame inside a `for` loop still finds the loop frame beneath the `if`, as a regression guard on the scan depth |
