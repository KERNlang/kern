...
| 2026-09-03 | **A high-risk review's finding, recorded rather than fixed: the list arm's `contract.kind === 'list'` guard is the type narrowing the spec pseudocode elided.** `crossCallExpressionType`'s list arm reads the element's cross-call contract and refuses when `contract === undefined \|\| contract.kind === 'list'`, which is exactly RT10X-C5's "ask the table instead of hard-coding two element names" and admits nothing beyond the `boolean`/`text` element rows the table carries. Functionally equal to what RT10X-C5 already decided; no behavior changed. |
| 2026-09-03 | **A high-risk review's finding, out of this slice's blast radius: `assertLinkLabel` (`scripts/kern-5-rt6-void-fallthrough/k0-support.mjs`) asserts `message.includes(label)`, so a mutant that extends a label string rather than replacing it survives in every slice from RT-6 onward.** Already recorded once in this document as the M08 test-strength finding; the review confirmed the same gap is structural to the shared RT-6 harness, not local to this slice's mutant battery. Closing it means turning the helper's substring check into a boundary-anchored one, which edits a prior slice's harness outside this slice's licensed blast radius. Queued as a shared-harness slice. |
| 2026-09-03 | **A high-risk review's finding, recorded rather than fixed: `contracts.ts` is over the 500-line ceiling.** It stands at 512 lines after this slice's row-level freezing (RT10X-C2's `integer` row plus the shallow-freeze correction below, the latter reformatted by `biome format` onto more lines). Splitting it is a source-file move under `packages/core/src`, which RT10X-C11 and *Builder must NOT* rule 12 both forbid here: no file may be added or removed under `packages/core/src` without re-attesting the compiled-core inventory pinned at `count: 354`. The split needs its own c-py-1 compiled-core transition entry and is queued, not performed in this slice. |
| 2026-09-03 | **A high-risk review's finding, recorded rather than fixed: deploy order for a source change under `packages/core/src` is `compiledCoreDigest` re-pin then `pnpm write:kern-canonicalizer-coverage`.** *Blast radius* already specifies this order and the reason — the re-pin edits a `.mjs` under `scripts/kern-canonicalizer`, which moves `coverageImplementationDigest` as a side effect, so the write pass must follow the re-pin rather than precede or replace it. Restated here because the review asked for it explicitly; no change to the documented order. |
| 2026-09-03 | **A high-risk review found the cross-call, binary-operator and unary-operator tables were shallow-frozen: `Object.freeze` on the table but not on its rows.** `LINKED_KIR_CROSS_CALL_TYPES.boolean.kind = 'text'` (and the equivalent on any binary or unary operator row) silently succeeded before this correction, because `Object.freeze` is shallow and the two resolvers this slice made table-driven (RT10X-C5) read those rows directly. Fixed by wrapping every row literal in its own `Object.freeze(...)` in all three tables; `satisfies Record<...>` is unchanged, and no row's value moved. One oracle test (`k0-golden.test.mjs`) now asserts a row mutation attempt throws in ESM's strict mode and that `linkedKirCrossCallType`'s answer is unchanged afterward. |
| 2026-09-03 | **A high-risk review found `crossCallExpressionType`'s user-call arm let the K0 golden dictate a spelling rather than a semantics.** The arm read `callee?.returnType.kind === undefined` solely so the golden's scrape of `returnType.kind` would match literal text inside the function body, coupling the implementation's exact spelling to the test rather than the other way around. Replaced with `callee === undefined`, which states the actual intent — "no callee, no cross-call type" — and lets `linkedKirCrossCallType(callee.returnType)` do the table lookup unconditionally once a callee exists. The K0 golden scrape was rewritten to match: it now requires `crossCallExpressionType` to contain the literal `linkedKirCrossCallType(callee.returnType)` and `staticExpressionType` to contain `returnType.kind`, describing what each resolver actually reads instead of pinning one shared substring. No golden JSON digest moved, because `recompute()` seals table shapes and admission outcomes, not resolver source text. |
| 2026-09-03 | **A high-risk review found the literal arm of `staticExpressionType` and `crossCallExpressionType` duplicated: boolean → `'boolean'`, canonical integer → `'integer'`, text → `'text'` (cross-call only), with the same `CANONICAL_INTEGER` guard copied into both functions.** Extracted a module-private `literalCrossCallType` helper in `expression.ts` that both resolvers call; `staticExpressionType` narrows away the `'text'` answer the helper can return, since `LinkedKernKirStaticType` has no text member. The two literal arms can no longer disagree by construction, which is a stronger reading of RT10X-C5's "cannot disagree on a literal" than the prior copy-guarded-by-the-same-regex form. `staticExpressionType` was deliberately left calling `crossCallExpressionType`'s user-call and identifier arms *pattern*, not delegating to it directly, because identifier arms read different scope maps (`scope.types` vs `scope.crossCallTypes`) and collapsing them would be wrong. The K0 golden scrape was updated to require both resolvers to call `literalCrossCallType(` and to require the helper's own body to contain `CANONICAL_INTEGER.test`, so the guard stays visible to the scrape without pinning where the guard's characters live inside the resolver text. |
