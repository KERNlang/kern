...
| A8-F7 | post-hoc sorting | Change the post-sort `orderedFactWrappers` emission loop start from `0` to `1`, omitting exactly one already-sorted link fact | Pristine output equals the M2 reference for the same captured 18-vector; fixture has at least two unequal ordered facts and the omitted first fact is identified | Independent M2 reference mismatch with successful runtime/decoder |
...
| A decoder-successful reversed canonical family can be killed only by the M2 reference. | The strict decoder rejects every distinct-key reversal before the independent oracle. A one-site post-sort loop-start defect instead omits one fact while preserving order and decoder validity. | F7 uses `orderedFactIndex from=0` to `from=1`; decoder success is mandatory and the M2 reference is the sole semantic killer. |
