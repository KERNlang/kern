...
| **[RT11W-TD1]** `WHILE_TWINS['twin-two-lets-assign']` is the straight-line twin of one `meter-trips-3` trip | `whileAccumulate` charges **two** statements per trip (`assign acc += term`, `assign i += 1`), but the twin fixture carried only the counter assign, short by one statement. Found only once `while` compiled: at base every fixture that reached this row failed to link at all, so the count-off-by-one was invisible until the loop admitted. Measured by hand: `tick-discipline.test.mjs`'s own `checkpoints()` gave `loop=8, twin=6` (diff 2, not 1), and the cross-check row gave `viaWhile−viaFor=2` against `increment−plain=1` | Added the missing `assign target="acc" value="acc + 1"` to the twin. Re-measured: `twin=7`, `loop−twin=1`; `increment−plain=2=viaWhile−viaFor`. The same fixture is shared by `metering.test.mjs`, which isolated the counter-increment's cost alone against it (`bodyCost('twin-two-lets-assign','twin-two-lets')=4`); with the fix that measures both statements together (`=8`), so its base-cost row and its `C`-derivation row are updated to use the combined body cost directly rather than double-subtracting the accumulator — re-measured `fromIncrement=fromNeverEntered=3` |
| **[RT11W-TD2]** The while lowering "adds no line to the JavaScript target kernel" is checked in part by asserting `parts.kernel.includes('__checkAbort')` | `__checkAbort` is defined inside the specialized handler (it closes over the request-scoped abort controller and deadline), not inside the shared kernel — true for `for` as well, and RT10F's own precedent test for this claim checks `__intValue` (genuinely kernel-resident), never `__checkAbort` | Dropped the `__checkAbort` assertion. `compatibility.test.mjs`'s `JAVASCRIPT_KERNEL_SHA256` pin already proves the kernel is byte-unchanged; this row now asserts the structural half only — zero `while(true)` in the kernel region, `__Fault` present there, and exactly one `while(true)` in the handler section for an unnested fixture |
| **[RT11W-TD3]** "A nested while carries one checkpoint per head, so nesting cannot lose one," asserted as `checkpoints(nested) − checkpoints(single) === 1` | `meter-nested-2x2`'s body carries statements a single loop's body does not (the inner counter's own `let j`, the outer's own `assign i`), so its emitted checkpoint count is not "single's count plus one head" — measured `nested=11, single=8`, diff 3, not 1 | Rewritten as a metering identity rather than a checkpoint census: widening `meter-nested-2x2`'s inner bound by one trip (new fixture `meter-nested-2x3`, identical shape, inner bound only) must cost exactly one single-loop trip's worth of steps (`ticks('meter-trips-1') − ticks('meter-trips-0')`) once per outer pass. Paired with a two-leg step-threshold identity on `meter-nested-2x2` (RT-1 vs JavaScript) in place of the discarded second identity, which could not be built cleanly from existing fixtures without a third nested shape |
| **[RT11W-TD4]** Blast Radius names six `RT2_GOLDEN_SHA256`-literal consumers (rt4, rt5, rt6, rt9, rt10-pre, rt10-X `compatibility.test.mjs`) as the complete licensed-cascade consumer set | A seventh consumer exists outside the named six: `scripts/kern-5-parity-ledger/frozen-surface.test.mjs`'s `NEIGHBOUR_GOLDENS` map pins `kern-5-rt2-boolean-if`, `kern-5-rt3-binary-expression` and `kern-5-rt9-linked-assign` at their pre-cascade digests under a "the neighbour K0 goldens do not move" claim — a claim slice A wrote before this slice existed, protecting against drift from the ledger mechanism alone, not from a later slice's licensed statement-union cascade. `pnpm test:kern-5-parity-ledger` failed on exactly this row once the licensed rt2/rt3/rt9 golden moves landed | Re-pinned the three moved entries to the same post-cascade digests already verified elsewhere (rt2 `5db55623…`, rt3 `969dd11b…`, rt9 `1a28f75c…`); left the other three (rt10-cross-call-integer, rt10-pre-linked-arithmetic, rt6-void-fallthrough) untouched, confirmed unchanged by direct digest measurement |
| **[RT11W-TD5]** Blast Radius licenses `ledger-schema.test.mjs`'s "ships with zero rows" assertion becoming "carries the while row," as the one place slice A asserted an empty ledger | A second, un-named site carries the same assumption: `scripts/kern-5-parity-ledger/ledger-aware-gates.test.mjs`'s "the checked-in ledger defers nothing, so the shared gates are inert today" asserted `loadParityLedger().rows` deepEquals `[]` and both kind sets deepEqual `[]`. `pnpm test:kern-5-parity-ledger` failed there once the while row landed | Rewritten to assert the ledger carries exactly the while row and `kinds.statement` is `['while']`; the position-inertness loop over slice A's own `for`-shaped `STATEMENT_POSITIONS`/`EXPRESSION_POSITIONS` fixtures is unchanged and still passes, because none of those fixtures contains a `while` |
| **[RT11W-TD6]** Two more un-named sites in `scripts/kern-5-parity-ledger/exhaustiveness.test.mjs` carried the same pre-this-slice assumptions | (1) `STATEMENT_KINDS` pinned the seven-member linked statement union verbatim, so "the linked unions carry the kind sets the mapping must cover" failed once `while` became the eighth member; (2) "every node kind the Python emitter lowers today is mapped lowered" asserted the deferred set is `[]`, which `while`'s ledger-licensed `'deferred'` mapping entry necessarily breaks | Added `'while'` to `STATEMENT_KINDS` in its sorted position (last); renamed the second test to "the deferred node kinds are exactly the ones the parity ledger carries" and changed its assertion from `deferred === []` to `deferred === ['while']`, matching `KIR_PYTHON_STATEMENT_LOWERING.while` and the ledger row exactly |
| **[RT11W-TD7]** `scripts/kern-5-parity-ledger/refusal-golden.test.mjs` assumed the checked-in ledger is empty and, separately, that any future row's nodeKind would be exercisable through the shared `STATEMENT_POSITIONS`/`EXPRESSION_POSITIONS` catalogues | Those catalogues are `for`/`binary`-shaped position sweeps (their own comments say so), so they carry no `while` node at all. "With an empty ledger every catalog-permitted position still compiles" asserted `ledgerRows() === []` before its loop even ran; "every ledger row refuses with the exact label" iterated the real `while` row against `STATEMENT_POSITIONS`, whose fixtures contain no `while` and so all reported `success`, not the refusal | Renamed the first test to "positions that carry no deferred kind still compile to Python" and dropped the empty-ledger precondition — its loop already proved the real claim (`for`/`binary` fixtures are unaffected by a `while`-only ledger). Added a `while`-shaped mirror of `STATEMENT_POSITIONS` (`WHILE_ROW_POSITIONS`, same five position keys, `while` as the vehicle with an explicit counter per RT11W-C14) and selected it by `row.nodeKind` in the second test, the same per-kind pattern the file's own synthetic tests 6/7 already use for `'for'` A third site, `scripts/kern-5-parity-ledger/parent-positions.test.mjs`'s "each ledger row refuses in every position of its surface," carried the identical bug; `WHILE_ROW_POSITIONS` was moved into the shared `ledger-support.mjs` (next to `STATEMENT_POSITIONS`) rather than duplicated a second time, and both test files import the one shared export |
