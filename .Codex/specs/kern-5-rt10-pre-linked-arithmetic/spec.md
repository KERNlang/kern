...
wraps only conversions that may exceed the active cap: `_lifted_digits(convert, argument)` uses
the stable process-wide `sys.__dict__["_kern_digit_window_lock"]` reentrant lock, reads
`sys.get_int_max_str_digits()`, sets 0, converts, and restores the previous value in a `finally`,
guarded with `hasattr` for < 3.11. The stable unversioned key is an inter-artifact invariant: every
KERN-generated module in one interpreter must resolve the same lock, including future emitted
versions. Canonical decimal text whose exact digit length is at most the current cap bypasses the
lock; integer-to-text conversions bypass it only when a conservative `bit_length()`-derived upper
decimal-digit bound proves they fit. A zero cap never takes an unlocked decision: it acquires the
shared lock, re-reads both the cap and KERN window state, and bypasses only for an externally
configured unlimited interpreter with no active KERN window. Unknown conversion shapes take the
locked path. The shared lock serializes large
windows from independently emitted modules and permits nested KERN conversions. Applied at every site the cap governs — `_int_value`'s
...
unrelated embedder thread that does not use the KERN lock may observe the lifted cap for the duration
...
| Date | Correction |
| --- | --- |
| 2026-09-04 | **Inherited F5 policy re-pinned to the reconciled base.** RT-10-pre's compatibility guard named the predecessor F5 policy digest `e025392a…`, but the reconciled RT-9/F5 base legitimately changes it to `0f62f6c9…`: F5's F4-policy binding moves, `maxWorkSteps` rises to the measured 3x cap, and `maxIterations` is added. The guard remains a freeze fence relative to that reconciled base; RT-10-pre itself still does not edit F5 policy, composition, or amendments. |
| 2026-09-04 | **The per-conversion digit window needed inter-module serialization.** Saving, setting, converting, and restoring the process-global CPython digit cap in each emitted module is nest-safe but races across independently emitted modules: one conversion can restore the prior cap while another still converts and the later restore can leak `0`. `_lifted_digits` now uses one reentrant lock obtained through `sys.__dict__.setdefault`, so every KERN-generated module shares it; a two-module threaded driver proves B cannot enter its conversion while A owns the window, both conversions observe `0`, nested conversion completes, and the prior cap is restored. This does not change the accepted external-thread observation window. |
| 2026-09-04 | **The shared lock must be stable and avoid ordinary conversion overhead.** The versioned `_kern_digit_window_lock_v1` name would let independently emitted future kernel versions use different locks, and locking every ordinary conversion serializes unrelated short KERN executions. The key is now the stable `_kern_digit_window_lock`; canonical decimal strings use exact length, and integer text uses a conservative upper bound from `bit_length()`, so only conversions proved within the active cap bypass acquisition. A driver-preinstalled instrumented lock proves short `int` and `str` conversions take no lock, while a large second module signals its acquisition attempt before blocking and cannot enter until the first releases. |
| 2026-09-04 | **A zero-cap fast path was a TOCTOU.** Reading `limit == 0` and `_kern_digit_window_active` without the shared lock permits B to see A's lifted cap, then see A's restored inactive marker and convert under the restored cap. Zero therefore always enters the stable lock and re-reads both values; only an external unlimited policy with no KERN window bypasses. The two-module oracle now freezes B after it has read A's zero cap, releases A, then resumes B and requires B's conversion to observe zero; a direct identity assertion proves both artifacts hold the same stable lock. |
