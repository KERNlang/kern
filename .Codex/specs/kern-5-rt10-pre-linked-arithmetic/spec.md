...
wraps each conversion instead: `_lifted_digits(convert, argument)` acquires the process-wide
reentrant lock stored in `sys.__dict__`, reads `sys.get_int_max_str_digits()`, sets 0, converts,
and restores the previous value in a `finally`, guarded with `hasattr` for < 3.11. The shared lock
serializes windows from independently emitted modules and permits nested KERN conversions. Applied at every site the cap governs — `_int_value`'s
