...
| `pnpm test:kern-5-rt10-pre-linked-arithmetic` | 13/13 — the pre-image reconstruction now also reverts the carried `rt2GoldenSha256` pointer the `for` slice moved, so it reproduces the frozen historical byte-record without recomputing it; see Corrections Log |
| `pnpm test:kern-5-rt9-linked-assign` | 7/7 — same fix, the reconstruction also strips the `for` admission row and statement-kind entry the `for` slice added; see Corrections Log |
