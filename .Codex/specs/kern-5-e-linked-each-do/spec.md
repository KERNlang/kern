...
6. `pnpm write:kern-canonicalizer-coverage`, then re-pin `compiledCoreDigest` in
   ...
   `coverage-summary.json`, **then run the write again**: `coverageImplementationDigest` hashes
   every `.mjs` under `scripts/kern-canonicalizer`, that test file included, so the hand re-pin
   invalidates the digest the first write recorded. The second write converges.
7. Full local gate:
...
| The Blast Radius listed four downstream pins (`STILL_OUTSIDE`, exhaustiveness, ledger, rt10 `neg-each`). **Twelve more moved**, all of them exact-set or digest rows that go RED the moment the union widens | Measured by running the neighbours: the parity ledger's three hard-coded row goldens (`ledger-schema.test.mjs:54`, `ledger-aware-gates.test.mjs:63` and its `deferredNodeKinds` list, `exhaustiveness.test.mjs:87`); the ledger's **parent-position sweep** (`parent-positions.test.mjs:69`, `refusal-golden.test.mjs:76`), whose generic catalogue is `for`-shaped and carries neither new node, so a fallthrough row makes the Python compile **succeed**; rt9's `control-each` admission and its pre-RT-9 pre-image reconstruction; rt2's `STATEMENT_PROBES`/`PROBE_BODIES`; rt11's `neg-while-each-in-body`; rt12's `statementSubBlocks` list; d0's statement-union row and both fault censuses; slice D's union rows, `LEDGER_NODE_KINDS`, KIR-token vocabulary, `STILL_OUTSIDE` row, fault carrier and relocated rt12 scrape; and the rt2 → rt3 → rt9 **licensed golden cascade** with its nine digest pins (`frozen-surface.test.mjs:46-57` names that cascade by design) | All moved minimally and by narrowing. New: `DO_ROW_POSITIONS`/`EACH_ROW_POSITIONS` in `ledger-support.mjs` (all five parent positions measured projecting and linking on both legs — `helper-body` included, because `list<text>` **is** a cross-call type, `contracts.ts:123`); rt10 fixture `neg-each-await`; rt11 fixture `neg-while-each-source-in-body`, which keeps rt11's body-attribution property with a live `KIR_EACH_SOURCE_NOT_PARAMETER` now that the leaf refusal is gone; rt2's `PROBE_PARAMETERS` override, because `each` is the only probe needing a list in the signature. Slice D's rows now import slice E's pins (`STATEMENT_KINDS_AFTER_E`, `DO_LABELS`, `EACH_LABELS`) instead of restating them, the pattern D already used for d0's fault deltas |
